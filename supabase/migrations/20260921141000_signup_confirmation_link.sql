...
begin
  ...
  -- Email/password signup inserts an unconfirmed auth row before confirmation.
  -- Keep its reserved catalog available for the subsequent confirmation update;
  -- ordinary signups and profiles that already own a catalog keep their behavior.
  if linked_teacher_id is null and new.email_confirmed_at is null
    and normalized_email is not null and exists (
      select 1 from public.teacher_catalogs teacher
      where teacher.profile_id is null and lower(teacher.account_email)=normalized_email
    ) then
    return new;
  end if;
...
-- Only the first confirmation transition completes a deferred signup. Other
-- account updates must not reapply signup metadata to an existing teacher.
create trigger on_auth_user_email_confirmed
  after update of email_confirmed_at on auth.users
  for each row
  when (old.email_confirmed_at is null and new.email_confirmed_at is not null)
  execute function public.handle_new_dashboard_user();

...
begin
  ...
  if p_paper_kind = 'transformed' then
...
create function public.worksheet_v3_export_contract_ready()
returns boolean
language sql
stable
security invoker
set search_path = ''
as $function$
  select to_regprocedure('public.record_worksheet_v2_export(uuid,uuid,jsonb,text,text,text,public.export_format,integer,text)') is not null;
$function$;

revoke execute on function public.worksheet_v3_export_contract_ready()
  from public, anon, authenticated;
grant execute on function public.worksheet_v3_export_contract_ready()
  to service_role;

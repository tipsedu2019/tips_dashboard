...
declare
  ...
  v_response jsonb;
begin
  if v_actor_id is null or v_request_key is null then
    raise exception 'registration_access_denied' using errcode = '42501';
  end if;

  if not (
    v_waiting_kind is null
    and v_class_id is null
    and v_retake_decision is null
  ) and (
    v_waiting_kind is null
    or v_waiting_kind not in ('current_class', 'current_term_opening', 'next_term_opening')
    or (v_waiting_kind = 'current_class') is distinct from (v_class_id is not null)
    or (v_retake_decision is not null and v_retake_decision not in ('required', 'not_required'))
  ) then
  ...
  where track.id = p_track_id and task.type = 'registration'
  for update of track;
  if not found then
    raise exception 'registration_access_denied' using errcode = '42501';
  end if;
  ...
  set waiting_detail_kind = v_waiting_kind,
      waiting_detail_class_id = v_class_id,
  ...
  perform dashboard_private.write_registration_track_event(
    v_track.task_id,
    p_track_id,
    'registration_waiting_details_saved_v2',
  ...
  insert into dashboard_private.ops_registration_mutations(
    ...
    v_actor_id, v_request_key, v_track.task_id, 'save_registration_waiting_details_v2', v_fingerprint, v_response
  );
  return v_response;
end;
$$;

create or replace function public.save_registration_waiting_details_v2(
  p_track_id uuid,
  p_waiting_kind text,
  p_class_id uuid,
  p_retake_decision text,
...
set search_path = ''
as $$
  select dashboard_private.save_registration_waiting_details_v2_impl($1, $2, $3, $4, $5);
$$;

revoke execute on function public.save_registration_waiting_details_v2(uuid, text, uuid, text, text) from public, anon;
grant execute on function public.save_registration_waiting_details_v2(uuid, text, uuid, text, text) to authenticated;

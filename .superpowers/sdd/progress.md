...
Supabase free-tier operations Task 4: source candidate 완료. audit UPDATE를 reversible diff/chain metadata로 분리하고 seven trigger·private helper·partial predecessor index·ACL/RLS 경계를 forward migration에 고정했습니다. source 15/15은 통과했으나 isolated DB harness는 reviewed baseline pointer 부재로 allocation 전 중단되어 pgTAP/EXPLAIN/two-client probe 및 final 승격은 실행하지 않았습니다.
...
대시보드 과학·명단 작업 1: 완료. 과학 통계를 영어·수학과 독립된 버킷으로 추가하고 학생 분포·수업 요약에 ID 중복 제거 및 이름/ID 안정 정렬된 이름·학교·학년 명단 계약을 제공했다. RED→GREEN 집중 테스트 14/14를 통과했고 독립 검토에서 Critical/Important/Minor 0건으로 승인됐다. 관리형 샌드박스의 `.git` 읽기 전용 제한 때문에 working-tree diff로 검토했다.
대시보드 과학·명단 작업 2: 완료. 과학 탭과 명시적 과학 매칭, 학생 분포 최하위 교차 행 및 개별 수업 행의 접근 가능한 명단 Popover, 수업 운영 전체 기본 접힘, 두 패널 외곽 Card 테두리 제거를 구현했다. 계약 테스트를 과학 분기·Popover 배치·정확히 두 개의 border-0·모드 전환 자동 펼침 금지까지 보강했으며 집중 40/40, 대상 ESLint, 공백 검사와 독립 재검토를 통과했다.
대시보드 과학·명단 작업 3: 완료. 중복 studentBreakdowns·주간 시수 계산을 버킷당 한 번으로 줄이고 명단 Popover dialog에 대상별 접근 가능한 이름을 추가했다. 최종 독립 재검토 Critical/Important/Minor 0건, 집중 41/41, 전체 Node 1807/1807, 대상 ESLint, 전체 공백 검사, `next build --webpack` 79/79를 통과했다. 새 빌드로 localhost:3000을 재시작하고 과학 전체/고등/초중등 빈 상태, 수업 운영 학년·선생님·강의실 기본 접힘, 학생 분포·개별 수업 명단, Esc·바깥 클릭·포커스 복귀, 20명 내부 스크롤, 390px 무가로넘침, 두 패널 0px 외곽 테두리, 콘솔 오류 0건을 확인했다.

...
| 통계 | 네 탭 및 조회 hook/cache/cancellation 코드; 운영 키보드/필터 재현 | 세 수정 후 로컬 desktop/mobile/keyboard, 대규모 drilldown·권한별 흐름 |
...
- 독립 작업 리뷰: 지침 및 세 UI 변경 모두 차단할 발견 사항 없음. 최종 브랜치 리뷰 결과는 별도 기록한다.
- **남은 검증:** 3025는 `/sign-in`에 있어 실제 운영 계정·데이터로 수정 화면을 확인하지 못했다. 로그인 요청을 남겼으며 fixture 검증으로 인증된 실제 API 통합을 대신했다고 주장하지 않는다. 운영 main/배포/DB 적용/실제 발송은 수행하지 않았다.
...
첫 세 수정의 검증이 끝나면 등록 상태 전이·동시성·no-send 경계를 다음 우선순위로 잡는다. 실제 저장이 필요한 검증은 로컬 fixture/격리 DB의 해당 업무 시나리오부터 재현하고 운영 조작과 구분한다. 위 표를 갱신하며 한 번에 관련된 작은 변경만 수행한다.

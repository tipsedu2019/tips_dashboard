...
Q-09의 후속 구현·검증은 [별도 회차 기록](quality-conflict-contract-2026-09-05.md)에 정리했다. Q-10의 업무 충돌 SQLSTATE 분리는 로컬 검증을 마쳤다. 다음 묶음은 등록 상태 전이·동시성·no-send 경계 검토다. 실제 저장이 필요한 검증은 로컬 fixture/격리 DB의 해당 업무 시나리오부터 재현하고 운영 조작과 구분한다. 위 표를 갱신하며 한 번에 관련된 작은 변경만 수행한다.

...
- 상태: 로컬 구현·검증 완료, PR #65 운영 반영 준비 중. 운영 DB 적용 및 앱 배포는 아래 순서에 따라 별도 확인한다.
...
PR CI에서 기존 등록 상세 조회 3건이 `free-tier-guardrails`에 걸렸다. 역사적 기준 `fad56ae5`, 운영 기준 `6f354aae`, 기능 HEAD를 대조한 결과 이 3건의 조회 체인과 fingerprint는 동일했다. 이전에 추가된 안전한 선행 조회 함수 때문에 occurrence의 predecessor hash만 바뀌었고, 이번 교재 컬럼 추가가 같은 함수의 검사를 다시 실행하면서 노출됐다.

검사기는 이미 허용한 선행 문장 추가를 다음 diff의 기준으로 사용하면 잘못 거부했다. 이를 재현하는 연속 커밋 테스트가 수정 전 실패하는 것을 확인했다. 예외 manifest를 변경하지 않고 역사적 기준→현재 기준→후보의 조회 위치를 모두 비교하도록 수정했다. 새 기준에서 추가된 문장을 가로질러 조회를 이동하는 경우도 계속 거부한다. 조회 체인 복제, 복원, 분기 이동에 대한 기존 회귀 검사를 유지한다.

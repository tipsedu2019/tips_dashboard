...
- 현재 결론: 인증 후 하단 높이 계산을 수정한 기록은 7절에 보존한다. 이후 사용자가 최소 10행 유지와 긴 내용의 스크롤을 승인하여 데스크톱 표 내부 스크롤·고정 헤더·외부 pager를 구현했다(8절). 새 UI의 실제 화면 재검증은 Browser URL 보안 정책 차단으로 미완료다. 자동 테스트와 이전 화면 관찰을 새 UI의 렌더링 증거로 혼동하지 않는다. 원격 배포나 운영 성능을 주장하지 않는다.
...
2026-08-30 당시에는 인증이 필요했다. 2026-08-31 사용자가 준비한 기존 관리자 세션으로 아래 재검증을 수행했다. 최소 페이지 크기는 이후 10행 유지로 결정되었다. 현재 남은 게이트는 변경된 스크롤 UI의 브라우저 재검증과 실제 네트워크 관측이다.
...
학생 행은 약 37px, 수업 행은 실제로 37~81px였다. 수업의 일정/선생님/강의실 여러 줄을 숨기지 않았으므로 단순 `34px × 행 수` 가정은 성립하지 않는다. 당시에는 더 작은 행 수를 검토했지만, 이후 사용자가 최소 10행과 스크롤 허용을 선택했다(8절). 첫 행 측정만으로 모든 수업 행의 무스크롤 수용을 보장하지는 않는다.
...
- 이 단계에서는 여러 줄 수업 및 작은 화면 선택 도구의 완전한 no-scroll이 미완료였다. 이후 승인된 스크롤 허용 기준은 8절에 별도로 기록한다.
- push·배포·DB migration 적용·운영 p50/p95 측정은 하지 않았다.

## 8. 2026-08-31 최소 10행 유지 후속 변경

### 승인 및 구현 범위

- 사용자 결정: 5행을 추가하지 않고 자동 10/15/20을 유지한다. 긴 수업 내용은 숨기지 않고 스크롤을 허용한다.
- 데스크톱 표 높이를 전체 pager·하단 여백을 제외한 영역으로 제한하고, pager는 스크롤 영역 바깥에 유지한다. 수동 10/15/20에도 같은 높이 제한을 적용한다.
- 표 내부의 중첩 overflow를 이 화면에서만 해제한다. 헤더의 `sticky`/`relative` 충돌을 제거하고 고정 열보다 위에 헤더가 오도록 z-index 및 불투명 배경을 지정한다. 공용 Table과 전역 shell은 변경하지 않는다.
- 내부 scrollTop을 보정한 원래 body 위치로 자동 행 수를 계산하여 스크롤 자체가 행 수/요청 범위를 바꾸지 않도록 한다.
- 키보드 포커스 가능한 이름 있는 영역, 페이지 전환 시 위로 복귀, 상세에서 목록으로 돌아올 때 내부 세로 위치 복원을 추가한다. 모바일 학생/수업 카드는 기존 페이지 스크롤을 유지한다.
- 매우 낮은 창에서는 표가 사라지지 않도록 최소 높이 160px을 유지하므로 페이지 자체 스크롤이 남을 수 있다.

### 자동 검증과 남은 화면 검증

- TDD RED: 신규 3개 테스트 실패(높이 함수·스크롤 구조 연결 부재), 기존 9개 통과. 구현 후 동일 파일 12/12 통과.
- 7절의 집중 명령 최종 재실행: 184/184 통과, 실패·취소·skip 0, 23.9초. 높이 768/900/952, 선택 도구로 tableTop이 증가하는 경우, 매우 짧은 창, 최소 10행 및 스크롤 위치 보정 계약을 포함한다.
- 전체 lint exit 0: 오류 0, 같은 기존 경고 6개.
- 최종 프로덕션 빌드 exit 0: compile 4.2초, TypeScript 통과, 정적 페이지 81/81. 독립 코드 리뷰에서 기능 차단 문제 없음; 최초 측정을 layout effect로 옮겨 클라이언트 첫 paint 전에 높이를 반영하도록 보완한 뒤 재리뷰와 수정 파일 ESLint도 통과했다.
- Browser는 사용 가능했으나 기존 localhost 탭 재접속이 URL 보안 정책에 의해 거부되었다. 외부 브라우저·별도 Playwright·다른 주소를 통한 우회는 하지 않았다.
- 따라서 이번 변경 후 페이지 식별/빈 화면/overlay/콘솔/스크린샷/상호작용 검사는 모두 미실행이다. 7절의 이전 관찰을 재사용하여 통과 처리하지 않는다.
- 남은 실제 검증: 1440×768/900/952의 학생·수업 기본/선택 상태, 표 내부 끝까지 스크롤 후 헤더·마지막 행·pager 접근, 다음 페이지 위로 복귀, 수동 20행, 상세 복귀 위치, 390×844 모바일.
- 새로운 DB 변경·원격 push·배포·데이터 수정·알림 발송 없음.

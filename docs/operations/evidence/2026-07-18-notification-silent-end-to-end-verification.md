...
상태: **운영 bridge 설치·무발송 검증 완료, 안전 코드 배포·관측 시작 대기, 새 시스템 실제 발송 비활성 유지**
...
- 2026-07-19 bridge 설치 문서 작성 시점에도 안전 코드는 `codex/notification-shadow-qa-20260718` 브랜치에 있다. 동일 코드를 `main`과 Vercel 운영에 배포하는 일은 다음 즉시 단계이며, 아직 완료로 판정하지 않았다.
- 운영 소유권 전환은 하지 않았다.
...
- legacy 계약 계측 bridge는 열림 상태(`closed_at is null`)다. traffic, outcome, 배포 영수증은 모두 0건으로 시작했다.
...
- `195000` 이후 closure·worker/watchdog schedule·provider claim·forward-compat·shadow 증거 마이그레이션은 적용하지 않았다. `pg_cron` 확장과 `cron.job`도 없다.
- 승인 범위는 위 bridge 설치, 같은 안전 코드의 `main` 배포, 무발송 계약 관측 시작까지다. shadow 활성화, worker 일정 설치, 소유권 전환, 실제 공급자 발송은 승인 범위 밖으로 유지한다.
...
bridge 적용 전 실제 관리자 화면의 최근 전달 요약은 마지막 전달 기록 없음, 대기 0, 완료 0, 실패 0, 결과 확인 필요 0이었다. bridge 적용 후 DB에는 보존 이력 93건만 존재하며 상태는 `sent` 20건, `skipped` 4건, `disabled` 69건이다. 새 발송 대기·재시도·처리 중·결과 불명 상태는 0건이다. 운영 배포 뒤 같은 화면은 다시 확인하되 연결 탭의 `테스트 메시지 보내기`는 누르지 않는다.
...
1. bridge와 같은 안전 코드를 `main`에 반영하고 Vercel 운영 revision·계약 hash 일치를 확인한다.
2. 배포 직후 첫 영수증부터 5분 간격으로 수집해 서울 기준 완결된 하루와 24시간 이상 legacy/canonical 자연 발생 비교를 통과한다.
3. 위 관측을 통과하고 별도 승인을 받은 뒤에만 `195000` 이후 legacy closure, worker/watchdog schedule, provider claim, forward-compat, shadow 증거 마이그레이션을 단계별로 검토한다.
...
이 문서 작성 시점에도 새 알림 시스템의 실제 발송 플래그는 켜지 않았으며, 새 시스템을 통한 Google Chat·Web Push·SOLAPI 실발송은 0건이고 모두 비활성 상태다. `main`·Vercel 배포와 배포 영수증 관측은 다음 즉시 단계이며 아직 완료로 기록하지 않는다.

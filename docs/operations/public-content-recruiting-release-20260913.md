...
기존 main의 마이그레이션 이력과 승인 seed는 원본 바이트를 유지했다. 이번에 처음 배포하는 두 migration은 SQL 검사에서 요구하는 transaction-local lock/statement timeout을 추가했고, CMS migration을 명시적 transaction으로 묶었다. 범위가 제한된 integer 열에는 해당 범위와 함께 좁은 Squawk 예외 주석을 달았다. 채용 cron 예약과 초기 파기는 자동 migration에서 제거하고 `scripts/operations/activate-recruiting-retention.sql`의 명시적 운영 활성화로 분리했다. 함수 본문·스키마 타입·권한·보관 기간은 바꾸지 않았다. 별도 migration, 운영 teacher 계정 변경, 자동 seed 실행, 이메일·메시지 발송 기능은 추가하지 않았다.
...
- 운영 활성화 분리 후 채용 DB 검증을 다시 수행했다. 설치 직후 cron과 정리 성공 기록이 없고 새 접수가 저장 없이 닫히며, 명시적 활성화 두 번 후에도 정리 작업이 하나만 유지되는 경계를 포함한다. 기존 operational migration guard는 그대로 유지한다.
...
| --- | --- |
| `20260911102816_recruiting_applications_private_intake.sql` | `e0b8d2c7b917bc9ecb47b2be57b55c4f5b0e4f0b40bbf83c97559cbefdc0e39c` |
...
3. 승인된 DB 절차로 `20260911102816_recruiting_applications_private_intake.sql`을 먼저, `20260912074228_public_site_content_management.sql`을 다음에 선별 적용한다. 전자는 pg_cron이 없으면 실패하며 테이블·권한·함수만 설치한다. 작업 예약·초기 파기는 실행하지 않으므로 정리 성공 시각이 없어 접수가 계속 닫힌다. 두 기능의 테이블은 새로 생성된다. 후자는 private `public-site-media` bucket을 생성한다.
...
8. 채용은 `RECRUITING_ALLOWED_ORIGINS`에 실제 폼 및 관리자 origin, `RECRUITING_RATE_LIMIT_SECRET`에 모든 인스턴스가 공유할 32자 이상 고정 비밀을 설정한다. Migration 성공 후 접수 flag가 false인 상태에서 승인된 DB 운영자가 `scripts/operations/activate-recruiting-retention.sql`을 실행한다. 이 별도 파일이 매시 13분 정리 예약과 초기 파기를 한 transaction으로 수행한다. 같은 운영자의 재실행은 작업을 중복 생성하지 않는다. `recruiting-retention-cleanup` 활성 상태·최근 성공 시각·다음 실제 실행을 확인하고 아래 IP 경계 검증을 마친 뒤 `RECRUITING_APPLICATIONS_ENABLED=true`를 켠다. 새 신청은 730일 보관의 `talent-pool-v2` 동의를 요구한다. 접수처는 `/admin/recruiting`이며 이메일 알림/전송은 없다.

...
An unknown transport/timeout retains the frozen request ID/body and exposes only an explicit result-confirmation retry. Newer local edits remain after that frozen generation. The confirmation retry first replays the identical request; after its receipt/result is acknowledged, later edits are rebased to the returned new revision and require another explicit Save. Known non-stale rollback errors retain all drafts but may form a new logical request after editing. A stale revision follows the explicit recovery below, never a silent reassignment of its base. Logout/actor or same-ID role change clears presentation/pending retry and rejects late acknowledgements; ordinary page/tab/filter changes preserve drafts for that actor.

For a known55000 revision conflict, retain every draft and show an inline conflict state with an explicit `초안 버리고 새로 불러오기` recovery action. Confirm that all unsaved owner/taxonomy changes will be discarded; cancellation changes nothing. Only confirmation clears the draft journals and reloads current null-draft pages/details/revisions for the same actor, preserving ordinary tab/filter intent. Disable editing/Save during this fresh-baseline load; failure stays a visible retryable read error, not an empty baseline. Never automatically replay an old journal against a new revision, which could overwrite another user's same-field edit without review. This discard action is unavailable while a save result is still unknown: resolve the same frozen request first. No new conflict-merging API or route-leave guard is introduced.

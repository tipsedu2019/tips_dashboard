...
After replay, assign contiguous global ranks to surviving rows on the server, writing only ranks that differ. Scalar updates must not reorder equal-rank ties accidentally before replay. Retain stable input ordinal for equal name-sort comparisons. Existing JS school sort uses `localeCompare('ko-KR', { numeric:true })`; the repository already has ICU `dashboard_private.ko_numeric` (`ko-u-kn-true`). Use that foundation with explicit tests for numeric names, punctuation/case, Unicode equivalents, raw draft whitespace, ascending/descending, and stable ties. PostgreSQL deterministic ICU tie-breaking is not automatically identical to JavaScript equality; test and resolve this explicitly, not by an undocumented lexical fallback.
...
Controller clarification: teacher baseRevision additionally hashes DISTINCT currently linked profiles visible through invoker RLS (`id,role,teacher_catalog_id,updated_at`); an unreadable link uses only a fixed sentinel keyed by its already-visible catalog profile_id. `expectedProfiles` covers newly selected IDs outside that initial revision, read through bounded ID context. Recompute after locks. Rank-shifted off-page linked teachers remain sync candidates; unreadable candidate profiles fail42501 because a no-op cannot be established. No hidden-data hashing or full profile transfer is allowed.
...
2. Acquire collection `SHARE ROW EXCLUSIVE` lock before re-reading/checking base fingerprint. This conflicts with normal INSERT/UPDATE/DELETE, including legacy direct writers; an advisory lock alone does not. Existing authenticated table ACLs contain DML privileges needed for this lock. Keep locks only for this short save transaction, never between page requests. For teachers coordinate profile locks before teacher locks (the existing signup path updates profiles before teacher catalogs); deterministic profile-ID order and bounded lock timeout. Verify lock-order interactions in database tests.

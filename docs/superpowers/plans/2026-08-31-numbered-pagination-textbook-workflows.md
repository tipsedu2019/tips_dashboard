...
- [ ] Start from the reviewed extraction checkpoint. Preserve the archived preliminary adapter work as provenance; restore its tests first and observe genuine missing-method RED before reusing or improving its implementation. Task2 final-wire fixtures and signed-decimal repair remain unchanged.
- [ ] Implement complete source predicates, deterministic purchase pairing and sale-history grouping before key pagination. Preserve exact raw-line facet loops, display-parent totals, server order, full-filter sale/operations totals, raw-status/teacher history semantics, full-source history options and stale-month fallback. Retain all real member mutation IDs and only required selected-parent enrichment.
- [ ] Keep teacher request page/summary/detail separate from admin/staff management reads under caller RLS. Test another actor's RLS-visible request, nonfirst-member detail, missing eligible source, role denial42501 and strict invalid inputs22023. Do not modify policies, writer functions or auth access.
- [ ] Add more than100 paired/grouped parents, repeated same-scope/timestamp ties, off-page quantity/facet/options, requested empty/out-of-range page counts, full scope invariance, missing/malformed API and cancellation/no-fallback fixtures. Run actual candidate then immutable final SQL with unchanged baseline parity and exact verbatim final-wire replay through the service.
- [ ] Only after final proof register these three exact numbered and six exact nonpageable contracts. Run affected service/model/ledger/workspace/budget tests, nonincremental TS and lint; commit as `feat: add numbered textbook purchase and sale reads`, report task-3b-report.md, then stop for independent review.
...
**Interfaces:** The audit's independent textbook/class/teacher/location search and selected-reference boundary is mandatory. Before dispatch, finish `.superpowers/sdd/2026-08-31-numbered-pagination-textbook-workflows/task-4-reference-preflight.md` with the source-backed signatures/filter/result shapes and add them here. This explicit design gate is owned by root while Task3 executes, not authority to guess a universal privileged catalog API. Ordinary reference search must be bounded and cannot depend on a record-list page; detail/alias lookup is independently complete. Existing whole-inactive cleanup needs a separately explicit complete target-ID/preview/count context, obtained only after that action is requested, with no actual purge during this task.

- [ ] Freeze source-backed APIs from actual TextbookSelect/ClassSelect/TeacherSelect/LocationSelect, their search/facet/metadata helpers and lifecycle alias resolver. Preserve exact→normalized→compact reference precedence and edition separation, missing/custom selected values, original location/default behavior and actual request-role source scope. Preserve active-only versus all-reference lookup meaning explicitly. No catalog cap proves completeness.

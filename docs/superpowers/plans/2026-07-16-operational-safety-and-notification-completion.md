...
### 작업 3: 수정 가능한 가짜 이력을 정직한 읽기 전용 이력으로 교체
...
- `buildRegistrationSubjectHistory(detail)`은 `actorKind`, `actorId`, `systemSource`, null을 허용하는 `occurredAt`, `timeKind`, `origin`을 포함한 항목을 만듭니다.
...
- 주요 단계는 최신순으로 정렬합니다.
- 공유 예약은 두 과목 배지를 가진 한 행으로 묶습니다.
- 현재 담당자와 사건 행위자를 분리합니다.
- 알림 내부 이벤트를 제외합니다.
- version-1의 null 행위자는 `알 수 없음`으로 표시합니다.
- 이전 자료는 `마이그레이션`으로 표시합니다.
- 이전 자료의 `timeKind`는 변경 가능한 `updatedAt`을 빌리지 않고 `unavailable`을 사용합니다.
- 이력 안에는 입력·수정·삭제·담당자·처리 예정일 제어가 없습니다.

- [x] **2단계: 닫힌 주요 단계 매핑 정규화**

`docs/superpowers/plans/2026-07-15-registration-appointments-reminders.md`의 작업 3에 정의된 정확한 사건-단계 매핑을 사용합니다. 세부 예약 변경은 주요 단계의 상세 메타데이터가 됩니다. 알림 사건·분배·전달·재시도·공급자 행은 절대 포함하지 않습니다.

- [x] **3단계: 이력 연결과 변경 가능한 현재 상태 분리**

`담당자 및 일시 이력` 접기 영역을 제거하고 canonical 등록 상세·편집기에 `RegistrationHistoryTimeline`을 연결합니다.

담당자와 다음 처리 시각이 계속 필요하면 현재 작업 버튼 옆의 별도 `현재 업무` 영역으로 옮깁니다. 이 영역은 변경 가능한 현재 상태이며 이력과 시각적·의미적으로 합치지 않습니다.

- [x] **4단계: 집중 테스트와 브라우저 검증 실행**
...
브라우저 검증에서는 두 과목 공유 이력이 있는 사례를 열고 과목별 필터와 상세 펼치기를 확인하며, 이력 변경 동작이 없음을 검증합니다. 공급자 호출은 0건으로 유지합니다. 병합·배포 준비 완료로 부르기 전에 전체 필수 배포 게이트도 실행합니다.
...
**이력 배포 게이트:** 사용자는 이력 행을 입력·변경·삭제할 수 없고 행위자·시각 표시는 변경 불가능한 사건에서 오거나 명시적으로 알 수 없음이어야 하며 전체 필수 배포 게이트를 통과해야 합니다.

**완료 근거:** 구현 커밋 `5667ad4`. 최종 집중 테스트 `87/87`, 전체 Node 테스트 `1213/1213`, TypeScript, 대상·전체 ESLint 오류 0건, 변경 공백 검사, 별도 임시 복사본 프로덕션 빌드의 정적 페이지 `75/75` 생성을 통과했습니다. 브라우저에서 읽기 전용 필터·상세, 원시 영문 상태값 미노출, `scrollWidth = viewportWidth = 1082`를 확인했습니다. 독립 검토 결과 P0/P1/P2는 `0/0/0`이며 실제 공급자 호출과 원격 변경은 없었습니다.

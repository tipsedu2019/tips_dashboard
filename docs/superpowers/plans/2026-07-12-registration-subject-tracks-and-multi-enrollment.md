...
Immediately after those locks, add `-- global_roster_projection_preflight`. Validate all four live JSONB columns globally: SQL null is normalized to empty only for inspection, every non-null value must be an array of UUID strings, arrays must be duplicate-free after normalization, each student/class pair must appear symmetrically on both sides in exactly one mode, and no pair may be both enrolled and waitlisted. Also require every student whose status is `퇴원` to have empty own-side arrays and no reverse class-side enrolled/waitlist reference; raise `registration_withdrawn_roster_review_required` for any violation. `재원` with no class remains valid because explicit registration cancellation may leave the student active. The current read-only snapshot has one student-side enrolled pair missing the reverse `classes.student_ids` link and no history row from which direction can be inferred; it is unrelated to the current registration parents, so the registration-only evidence scan would miss it. Do not assume the student side wins.

Under the literal marker `-- reviewed_roster_projection_repairs`, require an operator-reviewed portable set of `(student_id, class_id, target_mode)` rows, where target mode is `enrolled`, `waitlist`, or `removed`. Ignore literals whose student/class IDs do not exist in an empty/local environment, but for each pair that exists require exactly one reviewed target and update all four JSONB projections to that target under the locked student -> class order. Every actually corrected asymmetric pair always writes one labeled `student_class_enrollment_history` repair row, even when the chosen target matches the one currently visible side; use nullable `previous_mode` because the prior pair was untrusted and include reviewed-repair provenance in the memo. An unmatched live asymmetry raises `registration_global_roster_repair_required`; malformed JSON raises `registration_roster_projection_invalid`. After reviewed repairs, materialize and assert `global_roster_projection_symmetric = true` across every current pair. The current production migration remains blocked until the operator chooses the truth for that one pair; no production repair or migration application is authorized by this plan.

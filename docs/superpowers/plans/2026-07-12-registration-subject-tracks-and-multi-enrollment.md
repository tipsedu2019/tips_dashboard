...
- `complete_ops_withdrawal_roster_transition` replaces the existing client-side remove/status/checklist/rollback sequence for a withdrawal whose requested status is `done`. Completion is scoped to the selected class/subject: release only that roster pair and its optional active registration claim, preserve every other enrolled or waitlisted subject unchanged, and set the student to `퇴원` only when no other live roster projection or active registration claim remains. Otherwise keep the student `재원`. Resolve and lock the withdrawal task, selected-class claim parent, student identity, selected claim, and selected class in the established global order. Reject an open admission batch only when it owns the selected class. Require admin/staff access, a completed withdrawal checklist, the same locked student/source class as the task, and a symmetric enrolled source pair. A claimed enrolled row keeps `status = 'enrolled'` as history but sets `roster_active = false`, release timestamp/reason/source task, and `roster_release_kind = 'withdrawal'`; unrelated claims and tracks are not changed. Mark `timetable_roster_updated`, complete the task, and record the selected-class removal and resulting student status in the same transaction. Same-key replay returns the original committed response; any failure rolls back the selected removal, claim release, student status, and completion. Runtime fixtures cover multi-subject preservation, final-class withdrawal, source-class open-batch rejection, and rollback.

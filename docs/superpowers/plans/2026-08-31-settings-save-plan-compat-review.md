...
## Follow-up: empty no-op versus legacy global normalization

Read-only follow-up, 2026-08-31. All five UI `handleSaveAll` functions rewrite every surviving row's rank to **1..N** regardless of which field changed: school `:268–297`, classroom `:264–301`, term `:137–170`, class-group `:172–197`, teacher `:630–660` (all `src/features/management/*-master-workspace.tsx`). The service builders themselves do **not** compact supplied ranks: `management-service.js:754–816` preserves `sortOrder ?? sort_order`, falling back to zero-based input index only if absent. Consequently sparse/duplicate stored ranks become contiguous on any legacy UI Save that reaches the service.

The school UI collapses whitespace in every name; the other four trim every name, with additional builder/domain normalization. Schools/classrooms/terms/groups send the entire mapped array directly to upsert (`management-service.js:819–827,1544–1546,1589–1595,1603–1605,1613–1621`), so a successful Save can normalize an untouched off-page name and churn unchanged rows' timestamps. **Teacher exception:** `filterChangedTeacherCatalogPayload` compares normalized current and proposed values (`:663–699`, used at `:1554–1564`). An untouched teacher's whitespace-only name difference does not by itself force a write; a changed global rank or another changed field admits the whole normalized payload, which then also trims that name. It is inaccurate to say all five always physically normalize every name.

All five buttons disable Save for `!isDirty || saving` (school additionally blocks invalid names): school `:421`, classroom `:358`, term `:199`, class-group `:243`, teacher `:753`. However `isDirty` is a latched boolean set on field/actions, not a comparison with loaded state: e.g. school `:247–249`, term `:128–129`, teacher `:554–562`. Edit-then-revert, move-then-move-back, or add-then-delete can therefore enable a semantic no-op Save. Empty RPC requests also need a server contract independent of the UI gate.

**Recommended minimum explicit contract:** preserve order and user-field meaning, but intentionally do not preserve incidental legacy whole-array cleanup/timestamp writes. Normalize only fields explicitly patched and fields of new rows at final Save, plus required domain-coupled consequences (e.g. team change forcing assistant role); retain stored bytes of untouched fields, including off-page names. Rank-only changes must not normalize other fields. Compare the resulting final stored diff; empty/cancelled journals with no effect produce no catalog/profile DML or audit, though a retry receipt is allowed. Do not compact sparse/duplicate ranks merely because Save was called.

Keep existing ranks when final membership/order is unchanged **and those ranks, with the final field values under the normal read comparator, still produce the intended ID order**. The second clause matters: two rows can share `sort_order`, and renaming the first row can change the `name,id` tie-break on reload even though field edits never changed the draft array order. In that case rank reassignment is needed to preserve the draft's order. When composition/order genuinely changes, or rank reuse would violate it, assign global **1..N** on the server and write only differing ranks; preserve normal term year-first post-save behavior rather than trying to defeat it with ranks. Cancelled structural operations need not compact anything. Tests should cover sparse/duplicate ranks, untouched whitespace names, a scalar edit that keeps order, a tied-rank rename that would change read order, and a cancelled move/add/delete with zero timestamps/audit/profile churn.

**Document conflict:** audit `docs/superpowers/plans/2026-08-31-settings-save-contract-audit.md:55` currently mandates unconditional contiguous ranks after replay, while `:89` and settings workflow `:130` promise semantic/empty no-op without timestamp churn. These cannot both hold on a preexisting noncontiguous collection. Replace the unconditional rule with the conditional rank-reuse contract above and make explicit-field normalization scope explicit in audit `:61–69` / workflow `:102,133`. The root app-wide design `docs/superpowers/specs/2026-08-31-app-wide-numbered-pagination-design.md:63` requires deterministic ID-ended order, not unconditional contiguous ranks; this correction does not conflict with that root rule. This is an intentional narrowing of incidental legacy Save side effects, not exact persistence-side-effect parity.

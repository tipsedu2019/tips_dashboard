...
The reusable controller export is `createNumberedPageController<T>({loadPage,onChange})`. `loadPage({scope,page,pageSize,signal})` returns `Promise<NumberedPage<T>>`; `scope` is an opaque stable string containing viewer, list, filters and sort, with domain parsing owned by the adapter. `onChange(snapshot)` receives `{scope,requestedPage,page,pageSize,totalCount,rows,loading,error}`. Its methods are `load({scope,page,pageSize})`, `retry()`, `dispose()`. A new request leaves a successful snapshot's displayed page/size/rows together until success; requested page is separate. A new authorization scope must be cleared at the consumer boundary rather than showing another user's previous rows. Aborted/stale completions publish nothing. Out-of-range empty results refetch the final valid page once; concurrent further shrink reports a retriable error rather than loops. Domain hooks retain their metadata/detail/mutation logic.

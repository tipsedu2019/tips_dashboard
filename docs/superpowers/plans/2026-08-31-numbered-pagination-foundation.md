...
For the extracted brief, these exact shared signatures are repeated here. Export `DataTablePageSizePreference = 'auto' | DataTablePageSize` and `NumberedPage<T> = {rows:T[];page:number;pageSize:DataTablePageSize;totalCount:number}` from `src/lib/numbered-pagination.ts`. Export `DataTablePaginationProps = {page:number;pageSize:DataTablePageSize;totalCount:number|null;loading?:boolean;onPageChange:(page:number)=>void;pageSizeMode?:'auto'|'manual';onPageSizeChange?:(preference:DataTablePageSizePreference)=>void;ariaLabel?:string}`. The Select includes automatic mode and the three manual sizes, reflects pageSizeMode, and forwards `'auto'` unchanged. Loading disables navigation; ariaLabel labels this list's navigation. Hook `setPreference(preference:DataTablePageSizePreference)` switches auto/manual and persistence, while `setAutoPageSize(size:DataTablePageSize)` updates measured size only when already auto and never overwrites a manual preference. Name the hook result type separately from the preference union. Hydration/query ready remains required.
...
**Interfaces:** Consumes `NumberedPage`/`DataTablePageSize` from Task1. Produces `createManagementNumberedReadService({supabase}).readPage({kind,filters,page,pageSize,sort,signal})` returning a NumberedPage of current management list DTOs. Export `ManagementNumberedSort = readonly {id:string,desc:boolean}[]` with zero to two distinct allow-listed columns, matching existing primary/secondary table sort. Empty sort uses the existing per-kind default (students status/title, classes title, textbooks subject/title), always ending in id. Include every currently actionable header or explicitly keep derived non-sortable headers disabled rather than sort page-local results. Produces authenticated `list_management_numbered_page_v1(p_kind text,p_filters jsonb,p_page integer,p_page_size integer,p_sort jsonb)` returning JSON `{rows,page,pageSize,totalCount}`; p_sort is the validated array. It must resolve the default class period using the existing resolver at the hook boundary; do not invent a separate default rule.

- [ ] Read the final active `list_management_page_v1`, `get_management_stats_v1`, Korean numeric collation and related tests, including the subsequent list patch in `20260825152044_registration_admission_checklist_roster_consistency.sql` requiring `enrollment.status='enrolled' AND roster_active`. Write RED service tests that request page11 directly and assert one numbered RPC, exact filters/sort/size/page, timeout signal and retry(false), parse valid/malformed responses, no cursor fallback and no intermediary calls. Use a strict Supabase transport double, assert returned production DTOs and emitted request contract.
...
- Modify `src/hooks/use-data-table-page-size.ts` for explicit management legacy-preference migration and atomic manual/measurement transitions if needed; retain the shared public interface.
- Create `src/features/management/management-numbered-state.ts` if needed for executable async request tests.
- Put domain-independent request lifecycle in `src/lib/numbered-page-controller.ts` with `tests/numbered-page-controller.test.mjs`, so later adapters reuse the same state contract rather than copy the management lifecycle.
...
The reusable controller export is `createNumberedPageController<T>({loadPage,onChange})`. `loadPage({scope,page,pageSize,signal})` returns `Promise<NumberedPage<T>>`; `scope` is an opaque stable string containing viewer, list, filters and sort, with domain parsing owned by the adapter. `onChange(snapshot)` receives `{scope,requestedPage,page,pageSize,totalCount,rows,loading,error}`. Its methods are `load({scope,page,pageSize})`, `retry()`, `dispose()`. A new request leaves a successful snapshot's displayed scope/page/size/rows/count together until success; requested scope is internal and requested page is separate. Do not stamp old rows/count with a new scope. A new authorization scope must be cleared at the consumer boundary rather than showing another user's previous rows. Aborted/stale completions publish nothing. Out-of-range empty results refetch the final valid page once; concurrent further shrink reports a retriable error rather than loops. Domain hooks retain their metadata/detail/mutation logic.

Final-review correction: the `loadPage` request also provides `canonicalizeScope(scope:string):boolean`. An adapter may pin a resolved default period into the current internal target/retry scope before its numbered RPC; this publishes no snapshot and leaves previous successful rows/scope together. The callback is bound to that particular loadPage invocation and returnsfalse after that invocation resolves/rejects or becomes stale/aborted/disposed, including an earlier read during a later clamp invocation. Success and clamp use the pinned scope, and a failed numbered read retries that same resolved period. The public load/retry/dispose methods and NumberedPage response remain unchanged. Test late-callback rejection and actual hook defaultA -> canonicalA -> defaultB -> error/retry/refresh/clamp usingA without duplicate default resolution. Numbered row normalization must preserve the server's exact order; the legacy name-sort helper is not part of numbered paging.

- [ ] Write RED tests with deferred transports: page11 requested once without pages2–10; old page retained while pending/error; stale page/total rejected; filter+page reset atomic; refresh keeps valid page; delete-last-page clamps; arbitrary detail ID still loads independently; sort header changes server sort+page1, not just local loaded rows. Add a batched `setPreference(15); setAutoPageSize(20)` hook regression so delayed viewport measurements cannot overwrite the newly selected manual size before React commits; use atomic state transitions if the test exposes a stale-mode closure. Exercise an extracted production state/controller used by the hook rather than merely matching hook source.
- [ ] Change the hook's list execution to numbered reads while keeping independent stats/catalog/detail and mutations. Preserve canonical/default class period behavior, timed abort and stale scope protection. Add URL page/sort restore without overwriting unrelated detail/filter parameters. Sanitize persisted/URL sorts against Task2's exported allow-list before requests; unsupported previously saved derived sorts must fall back to a valid per-kind default, not leave the screen in validation error. No effect may publish old page rows with the new page number.
- [ ] Set TanStack manualPagination/manualSorting for server lists, use totalCount rather than prePaginationRowCount for pagination, remove load-more/continuation and local accumulated-row page controls. Wire the shared pager and wait for its page-size hook's ready flag before any list query. Preserve existing table-scroll bounds, auto measurement, per-kind manual preference and mobile cards. Migrate an existing valid `managementPageSizeStorageKey(kind)` value `{version:1,size:10|15|20}` when no new shared preference exists; do not silently reset an existing manual choice. Selection clears on displayed page/scope change; existing explicit mutation confirmations remain. Include auth identity/role in query scope and clear prior rows immediately on authorization changes.
...
**Files:** Update `docs/qa/2026-08-31-numbered-pagination.md`; preserve other plans' QA records. Modify `src/lib/query-surface-budget.js` and `tests/query-surface-budget.test.mjs` only for the verified numbered-RPC and immutable abort-signal recognition gaps described below.
...
- [ ] First reproduce query-surface budget's two false positives on the real management numbered service (`list_abort_signal_missing`, `rpc_page_limit_missing`). Write RED regressions and extend static recognition soundly: lexical immutable-const abort aliases by binding identity, visibility, no later write and cycle protection; keep the existing exact-two/no-spread AbortSignal.any rule and require every conditional branch to retain a bounded timeout. Reject unsafe caller-only aliases, shadowed unsafe bindings, let/reassignment, destructuring/property aliases and cycles. Do not merely exempt the service or relax timeouts.
- [ ] Include the integrated hook's immutable requestSignal and module-level `MANAGEMENT_TABLE_TIMEOUT_MS = 8000` aliases in the same proof. Resolve exact numeric8000 through visible immutable const chains and lexical ancestors; never skip a nearer shadow/parameter/destructured/mutable binding to choose an outer safe constant. Keep arbitrary properties/arithmetic unsupported. Add actual-hook and function/block/module-alias regressions plus unsafe size/value/shadow/write/cycle cases; the deadline remains exactly8seconds.
- [ ] Cover dependency-only changes outside query-function changed lines. If the corresponding base raw query diagnostics had no abort violation but head introduces `list_abort_signal_missing`, include it even when only an ancestor timeout/alias declaration changed. Reuse exact chain/occurrence matching, fail closed when matching is uncertain, and preserve existing legacy/scalar suppression policies. Test module8000→12000, safe→mutable/unsafe alias, deleted declaration, safe8000 alias change and unrelated module edits with unchanged legacy debt. Do not add a generic dependency-span framework for this narrow gap.
- [ ] Add a narrow numbered-RPC contract registry separate from scalar-RPC exemptions. Register only `list_management_numbered_page_v1` with required `p_page_size` and strict10/15/20 final-SQL contract already proven by pgTAP. Reject missing, duplicate and spread parameter envelopes and known invalid literal sizes; permit a dynamic page size only under this exact server-validated contract. Unrelated RPCs still require their existing p_limit contract. New registry entries require equivalent final migration and SQL proof. Test actual management source, safe alias chains, invalid9/11/31 versus valid10/15/20, unsafe shadow/reassignment, duplicates/spreads/missing size, and unrelated RPC behavior. Run the full query-budget suite without weakening its existing cases.
- [ ] Run combined new/management tests, query-surface budget, TypeScript, full lint and production build. Keep the running localhost3017 .next artifact untouched: use an isolated temporary source copy and existing dependency symlink for compile validation, never serving it as a Browser-policy workaround. Record exact results; do not treat existing unrelated lint warnings as new failures.

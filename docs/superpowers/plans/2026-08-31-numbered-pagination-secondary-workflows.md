...
- Modify `src/features/operations/operations-read-service.js` only if needed for narrow actor-session catalog-cache invalidation that also rejects late in-flight cache writes; preserve existing reader contracts and SQL authority.
...
- [x] Write RED controller/hook cases for page11 one read, atomic filter/size reset, retained rows on error, stale count rejection, refresh clamp and URL/detail return. Assert timetable/calendar/annual modes use their existing range readers, not numbered APIs.
...
- [x] Replace curriculum/schedule accumulation and list `loadMore` with shared controller and pager. Use successful response page/count, preserve scope metadata and all current filters. Keep `get_academic_curriculum_detail_v1` and `get_operations_class_lesson_design_detail_v1` independent of loaded rows.
- [x] Remove curriculum's requested-scope mismatch hiding/skeleton behavior when a successful displayed page exists. Rows/count/header labels stay on that displayed scope during pending/error. Scope operations by resolved `useAuth().role` plus actor, not app_metadata.role alone; clear controller, rows, metadata and catalogs on actor/role/logout transitions. Keep operations catalogs independent of page reads and available to unchanged range modes instead of waiting for catalog hydration on every page.
- [x] Cover logout then return to the same actor/role while an old catalog request is pending: the current module-level30-minute catalog cache must not accept that old completion or rehydrate prior-session values. Use narrow generation/session invalidation if needed and test the real service cache together with the consumer, not only a stubbed catalog array.
- [x] Feed operations group chips from returned syncGroupCounts and filterOptions, with totalCount for 전체 and only representativeClassId for its existing local-selection/lifecycle action. No full group members are required by current mutations/exports. Keep explicitly page-only indicators page-only and replace curriculum's overall row summary with full filtered total.
- [x] Preserve `lessonProgressDraft`, selected-class mutation lifecycle token and unsaved guards. Preserve scroll and URL parameters without replacing unrelated detail links. Update any visible total derived from loaded array to full-filter count while leaving per-page selection separate.
...
- [x] Run new tests, existing academic/operations scoped and continuous-class-schedule regressions, focused lint/TS, then commit as `feat: paginate curriculum and class planning lists`.
...
- [x] Copy existing tab predicates exactly from approval-workspace into test fixtures before moving them server-side. RED tests exercise all views, direct detail outside loaded page, page11 single RPC, failed-page retry and full tab counts. SQL tests cover involved/admin visibility, unrelated user exclusion, anon denial, updated_at ties with id DESC, selected-page-only comments/events and empty count.
- [x] Generate migration. Use invoker security, RLS and authenticated grants from final approval chain (base `20260523190000_approval_requests.sql` plus later definitions), order `updated_at DESC,id DESC`. Derive tab counts from authorized rows independent of chosen view; enrich only paged request IDs. Direct detail uses the same visibility boundary.
- [x] Separate profiles/templates loading from list pages. Replace in-memory complete-list/tab count assumptions with service/controller. Deep links resolve directly, choose owning tab from returned record and open it without scanning pages. Preserve sessionStorage idempotency, composer/edit draft guards, approval/comment/send mutation semantics unchanged.
- [x] Render an outside-page approval detail in a separate detail region/dialog, not by prepending it to numbered rows. Preserve mine across statuses and the existing review/open `isClosedApproval` predicates. The exact new metadata field is `tabCounts` (not the older audit's viewCounts); align service/RPC/tests on this contract.
- [x] Exercise unresolved auth, logout/user switch and same-ID role change through the real approval consumer with delayed page/detail/catalog responses. Prior-authority rows, tab counts and editor/detail state must not reappear; preserve drafts only within their current actor scope.
- [x] Preserve composer/checklist/template and per-request unsent comment drafts across page/tab replacement within the same actor; lift comment drafts out of row-local state so unmounting a page does not discard them, and clear only acknowledged matching saves. Keep the existing approval mutation receipt/retry semantics, but partition its module/sessionStorage attempts by authenticated actor so an uncertain prior user's create/update continuation cannot be adopted by the next user with identical input. Reject late old-session persistence/continuation and UI updates after actor transitions; do not alter SQL mutation authority, receipt IDs for same-actor retries, status transitions or notification producers. Add actual service/consumer tests for uncertain-save actor switch and page-away/back comment preservation. See the source-only task-3-consumer-preflight.md before implementation.
- [x] Treat a live legacy unscoped mutation attempt as unknown-owner evidence: do not automatically assign it to the current actor, delete it and silently issue a fresh duplicate write, or invoke a mutation to probe ownership. Preserve/quarantine it and show an explicit recovery path before starting an affected replacement attempt; require an intentional user decision to discard ambiguous local retry state, without deleting/changing any server document. Known actor-scoped retries remain automatic identity reuse within the existing explicit Save action. No new privileged receipt-read API is needed.
- [x] Add shared pager/preferences `approvals:requests`, preserve previous displayed state on error, reset/clamp correctly, and use an explicit error/retry path rather than replacing prior rows with an empty success. Run service/controller/SQL tests and existing approval tests, lint/TS, then commit as `feat: paginate approval request lists`.
...
- Modify `src/features/makeup-requests/makeup-request-model.js` and its declaration only for optional independently authorized active-event request IDs in the existing collision model; preserve legacy complete-request behavior when that new context is absent.
...
Collision context is independent of numbered rows. Add `readMakeupReservationContext({slots,eventRequestIds,signal})` backed by an invoker `get_makeup_reservation_context_v1(p_slots jsonb,p_event_request_ids uuid[])`. Return complete minimal authorized active reservation projections overlapping the requested time slots across all rooms, plus authorized active IDs among the explicitly relevant academic-event request IDs. Preserve the existing `buildRoomAvailability` collision semantics without loading complete request DTOs/events/history or limiting collision evidence to the current page. This is purpose-scoped read context, not another list UI; never silently truncate or treat failed/pending context as an available room. Existing mutation RPCs and their final authorization/locking/notification semantics remain unchanged.
...
- [ ] Preserve off-page room-collision warnings and approval/form gating using the separate reservation context, not `data.requests` containing only numbered rows. Query the union of accepted-page/detail/form time slots as needed; isolate actor+role and query generations, retain correctly scoped evidence, and disable collision-sensitive actions until a matching context succeeds. Preserve active statuses, self-exclusion, half-open time overlap, all-room suggestions and orphan-event filtering via independently checked active IDs. Use the existing regular-class/academic-event catalogs and pure collision model; do not redesign scheduling or weaken server mutation checks. Add real consumer/service/SQL tests with the conflicting request on page11 while its target is on page1, off-page edited detail, pending/failure, teacher/assistant RLS and orphan-event cases. Read `.superpowers/sdd/2026-08-31-numbered-pagination-secondary-workflows/task-4-display-and-collision-preflight.md` for exact source traps before implementation.
...
- [ ] Close the spec's local OFFSET plan-evidence gate with first/middle/final-page EXPLAIN plans for the four new list RPCs against valid synthetic authorized fixtures. Reuse the existing isolated runner and proven fixture setup; preserve raw plan artifacts in this plan's workspace and state actor/role, row counts, warm-cache/synthetic limits, and whether nested enrichment plans are actually visible. A wrapper-only plan is not nested-loop or production-latency proof. Do not add speculative indexes, change final migration bytes, bypass RLS, rerun optional instrumentation experiments, or claim constant-time offsets. A test-only self-contained probe under `tests/` may be added if required by the existing runner's staging contract; it must not execute DB work during ordinary unit-test discovery.

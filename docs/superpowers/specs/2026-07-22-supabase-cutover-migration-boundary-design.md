...
- 일반 DB 배포 CI는 다시 녹색이 되고 운영 DB에는 새 SQL이 적용되지 않는다.
...
6. DB push workflow가 검증기를 push보다 먼저 실행
7. workflow가 quarantine 경로를 복사·이동·직접 실행하지 않음
...
이번 변경은 Git 파일 배치, 정적 검증기, CI, 테스트와 문서만 바꾼다.

- 운영 DB DDL/DML: 없음
- migration history 변경: 없음
- runtime flag 변경: 없음
- cron/worker/watchdog 설치: 없음
- Google Chat/Web Push/SOLAPI 호출: 없음
- Vercel 앱 동작 변경: 없음

배포 뒤 `supabase migration list`와 `db push`가 active lane 기준으로 동기화되고, 원격 schema fingerprint와 provider-zero 상태가 작업 전후 동일해야 한다.
...
- 원격 schema fingerprint, flag, connection, pg_cron 상태 불변

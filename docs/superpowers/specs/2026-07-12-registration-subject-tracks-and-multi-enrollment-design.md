...
- Before backfill, null/blank, unknown, and mixed recognized/unknown legacy subject values require an explicit reviewed `(task, subject)` mapping. They are never filtered away or inferred from counselor/class metadata. The only exception is an explicitly reviewed empty UI fixture with no business evidence: its task/event history is quarantined as canceled `general` and its empty registration detail is removed. The migration aborts if any remaining registration parent lacks exactly one or two resolved subjects, and readiness is installed only after every remaining registration parent has the exact corresponding track set.

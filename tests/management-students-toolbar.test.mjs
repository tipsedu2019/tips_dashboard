...
test("management search preserves Korean IME composition before syncing URL filters", async () => {
  ...
  assert.match(tableSource, /const \[searchComposing, setSearchComposing\] = useState\(false\)/);
  assert.match(tableSource, /composing:\s*searchComposing/);
  ...
  assert.match(tableSource, /reconciliation\.pendingSearch === null && !searchComposing && globalFilter !== requestedSearch/);
...
test("student and class tables expose bulk edit and delete actions for selected rows", async () => {
  ...
  const bulkSource = await readFile(new URL("src/features/management/management-bulk-actions.tsx", root), "utf8");
  assert.match(bulkSource, /type BulkEditField/);
  assert.match(tableSource, /const BULK_EDIT_FIELDS/);
  assert.match(bulkSource, /function ManagementBulkActionBar/);

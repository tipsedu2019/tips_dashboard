...
test("google chat route keeps webhook URLs server-side", () => {
  ...
  assert.match(apiRouteSource, /const configured = isAllowedGoogleChatWebhookUrl\(resolvedWebhookUrl\)/);
  assert.match(apiRouteSource, /maskedUrl: configured \? maskAllowedGoogleChatWebhookUrl\(resolvedWebhookUrl\) : ""/);
  assert.match(apiRouteSource, /replaceLegacyGoogleChatConnection/);
  assert.match(apiRouteSource, /serviceClient\.rpc\("replace_google_chat_connection_v1"/);
  assert.match(apiRouteSource, /p_actor: input\.actorUserId/);
  assert.doesNotMatch(apiRouteSource, /client\.rpc\("replace_google_chat_connection_v1"/);
  assert.match(apiRouteSource, /p_webhook_url_ciphertext: input\.webhookUrlCiphertext/);
  assert.doesNotMatch(apiRouteSource, /\.upsert\(/);

...
test("Google Chat 408은 POST 이후 수락 여부가 불명하므로 재시도하지 않고 unknown으로 닫힌다", async () => {
  const { createGoogleChatProvider } = await import(googleChatProviderModuleUrl)
  const provider = createGoogleChatProvider({
  ...
  assert.equal(outcome.providerResponseCode, "408")
  assert.equal(outcome.nextAttemptAt, null)
})

test("Google Chat 425는 명시적 사전 거절이므로 재시도 대기로 남긴다", async () => {
  const { createGoogleChatProvider } = await import(googleChatProviderModuleUrl)
  const provider = createGoogleChatProvider({
...
test("Web Push 408은 POST 이후 수락 여부가 불명하므로 재시도하지 않고 unknown으로 닫힌다", async () => {
  const { createWebPushProvider } = await import(webPushProviderModuleUrl)
  const provider = createWebPushProvider({
...
test("Web Push 425는 명시적 사전 거절이므로 재시도 대기로 남긴다", async () => {
  const { createWebPushProvider } = await import(webPushProviderModuleUrl)
  const provider = createWebPushProvider({

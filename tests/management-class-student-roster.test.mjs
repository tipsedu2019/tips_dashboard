...
test("class detail focuses requested curriculum and schedule targets from URL", async () => {
  ...
  assert.match(pageSource, /return sessionId \? `class-schedule-session-\$\{sessionId\}` : "class-schedule-session-create-work-panel"/);
  assert.match(pageSource, /id="class-schedule-session-create-work-panel"/);
  assert.match(pageSource, /id=\{`class-curriculum-session-\$\{firstUnassignedSessionId\}`\}/);
  assert.match(pageSource, /id=\{`class-schedule-session-\$\{getCurriculumSessionStableId\(session\)\}`\}/);
  ...
  assert.match(pageSource, /data-class-detail-focused=\{shouldHighlightScheduleCreation \? "true" : undefined\}/);
...
test("class schedule detail tab shows official session and exception context", async () => {
  ...
  assert.match(pageSource, /회차 흐름/);
  assert.match(pageSource, /보강·휴강·예외/);
  assert.match(pageSource, /회차 생성·수정/);
  assert.match(pageSource, /data-testid="class-schedule-session-create-work-panel"/);
  assert.match(pageSource, /sessions\.length <= 0 \? "회차 미생성" : "회차 생성됨"/);
  assert.match(pageSource, />\s*회차 생성\s*<\/Button>/);
  assert.match(pageSource, /\{renderClassSchedulePanel\(\)\}/);
  assert.match(pageSource, /data-testid="class-schedule-exception-work-panel"/);
  assert.match(pageSource, /data-testid="class-schedule-exception-create"/);
  assert.match(pageSource, /data-testid="class-schedule-exception-edit"/);
  assert.match(pageSource, /section: "lesson-design-periods"/);
  assert.match(pageSource, /sessionId: getCurriculumSessionStableId\(session\)/);
  assert.match(pageSource, />\s*예외 등록\s*<\/Button>/);
  assert.match(pageSource, />\s*예외 수정\s*<\/Button>/);

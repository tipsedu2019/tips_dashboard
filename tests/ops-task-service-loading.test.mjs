...
test("normalized class detail retains legacy billing metadata for withdrawal calculations", async () => {
  const selections = [];
  const normalizedSession = {
    id: "session-makeup",
    session_key: "session-makeup",
      ...
      from(table) {
        return {
          select(columns) {
            ...
            if (table === "classes") {
              return {
                eq(column, value) {
                  ...
                  assert.equal(value, "class-normalized");
                  return this;
                },
                async limit(count) {
            ...
            return {
              eq(column, value) {
                assert.equal(column, "class_id");
                assert.equal(value, "class-normalized");
                return this;
              },
              async order(column) {
                assert.equal(column, "session_date");
  ...
  const detail = await loadDetail("class-normalized", { viewerId: "viewer-a" });
  const sessionSelection = selections.find(({ table }) => table === "class_lesson_sessions")?.columns || "";

  assert.match(sessionSelection, /legacy_billing_id/);
  assert.match(sessionSelection, /legacy_billing_label/);
  assert.match(sessionSelection, /legacy_billing_color/);
  assert.equal(detail.schedulePlan.sessions[0].legacy_billing_id, "period-august-2026");
});

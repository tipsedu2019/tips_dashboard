...
test("credentialed registration fixture verifier executes create, replay, reopen, and canonical edits", async () => {
  ...
  assert.match(verifier, /"\*\*\/api\/web-push"/)
  assert.match(verifier, /"\*\*\/api\/solapi"/)
  assert.match(verifier, /"\*\*\/api\/solapi\/\*\*"/)
  ...
  assert.match(verifier, /notificationTargetHistory/)
  assert.match(verifier, /target_reconciliation/)
  assert.match(verifier, /superseded/)

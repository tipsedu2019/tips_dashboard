...
test("provider-zero lane은 모든 golden identity를 렌더링하고 Google Chat만 정확한 가짜 payload로 보낸다", async () => {
  ...
  assert.equal(evidence.retiredGoogleChatIdentityCount, 28)
  assert.equal(evidence.fakeFormattingTransportCallCount, evidence.googleChatIdentityCount - 28)
  assert.equal(evidence.exactPayloadCount, evidence.googleChatIdentityCount - 28)

...
test("SQL review workflow는 PR base/head SHA로 immutable-final boundary를 호출한다", async () => {
  ...
  for (const focusedRegistrationContract of [
    "registration_archived_subject_delivery_fence_test.sql",
    "registration_manager_write_and_integrity_fence_test.sql",
    "registration_management_notification_explicit_v2_test.sql",
  ]) {
    assert.match(
      workflow,
      new RegExp(
        `--test supabase/tests/${focusedRegistrationContract.replaceAll(".", "\\.")}`,
        "u",
      ),
      `${focusedRegistrationContract} must run in the isolated PR schema contract`,
    )
  }
...
test("layout verifier pins every semantic predicate in the fixed postdeploy catalog query", async () => {
  ...
  const requiredPredicates = [
    ...
    ["observation enter public signature", "public.enter_registration_observation_v1(uuid,integer,text)"],
    ["observation enter private signature", "dashboard_private.enter_registration_observation_v1_impl(uuid,integer,text)"],
    ["retired notification v1 signature", "public.ensure_registration_workflow_notification_v1(uuid,integer)"],
    ["explicit notification v2 signature", "public.ensure_registration_workflow_notification_v2(uuid,integer,text,text)"],
    ["notification v2 event-key signature", "dashboard_private.registration_management_notification_event_key_v2(text)"],
    ["notification v2 snapshot signature", "dashboard_private.registration_management_notification_fact_snapshot_v2(uuid,uuid)"],
    ["notification v2 stale snapshot overload absent", "dashboard_private.registration_management_notification_fact_snapshot_v2(uuid)'"],
    ["notification v2 checksum signature", "dashboard_private.registration_management_notification_fact_checksum_v2(jsonb)"],
    ["notification v2 current-source signature", "dashboard_private.registration_management_notification_source_current_v2(uuid,uuid)"],
    ["notification v2 suppression signature", "dashboard_private.suppress_registration_management_notification_source_v2(text,text)"],
    ["manager actor signature", "dashboard_private.registration_actor_is_active_manager_v1(uuid)"],
    ["manager actor assertion signature", "dashboard_private.assert_registration_actor_is_active_manager_v1(uuid)"],
    ["manager replay assertion signature", "dashboard_private.assert_registration_task_replay_access_v1(uuid)"],
    ["manager write fence signature", "dashboard_private.enforce_registration_manager_write_fence_v1()"],
    ["appointment integrity signature", "dashboard_private.assert_registration_appointment_integrity_v1(uuid)"],
    ["appointment track integrity signature", "dashboard_private.assert_registration_appointment_integrity_from_track_v1()"],
    ["archive active-subject signature", "dashboard_private.registration_appointment_has_active_subject_v1(uuid,uuid)"],
    ["archive synchronous cancellation function absent", "pg_catalog.to_regprocedure(\n    'dashboard_private.cancel_registration_archived_subject_delivery_v1()'\n  ) is null"],
    ["common compatibility signature", "dashboard_private.update_registration_case_common_with_reminders_v1_impl(uuid,text,text,text,text,text,text,timestamp with time zone,text,text,integer,text)"],
    ["unified inquiry signature", "dashboard_private.save_registration_case_inquiry_v1_impl(uuid,text,text,text,text,text,text,timestamp with time zone,text,text,integer,text[],text[],text)"],
    ["subject sync signature", "dashboard_private.sync_registration_case_subjects_impl(uuid,text[],text)"],
    ["notification recorder signature", "dashboard_private.record_registration_management_notification_v1(uuid,text,uuid,uuid,bigint,timestamp with time zone,uuid)"],
    ["flat create private signature", "dashboard_private.create_registration_case_impl(text,text,text,text,text,text,timestamp with time zone,text[],text,text,text)"],
    ["flat create public signature", "public.create_registration_case(text,text,text,text,text,text,timestamp with time zone,text[],text,text,text)"],
    ["flat create legacy signature", "public.create_registration_case_with_initial_workflow_v1(text,text,text,text,text,text,timestamp with time zone,text[],text,text,jsonb,jsonb,jsonb,jsonb,text)"],
    ["collaboration guard signature", "dashboard_private.enforce_registration_collaboration_write_role_v1()"],
    ["feedback submit public signature", "public.submit_registration_observation_feedback_v1(uuid,text,text,text,bigint,bigint,integer,text)"],
    ["feedback submit private signature", "dashboard_private.submit_registration_observation_feedback_v1_impl(uuid,text,text,text,bigint,bigint,integer,text)"],
    ["feedback correction public signature", "public.correct_registration_observation_feedback_v1(uuid,text,text,text,bigint,bigint,text,text)"],
    ["feedback correction private signature", "dashboard_private.correct_registration_observation_feedback_v1_impl(uuid,text,text,text,bigint,bigint,text,text)"],
    ["checklist delegation", "dashboard_private.set_registration_admission_checklist_item_v1_impl%"],
    ["workflow status row mutation", "definition !~* ('up' || 'date[[:space:]]+public[.]ops_registration_subject_tracks')"],
    ["workflow audit event", "definition not like '%dashboard_private.write_registration_track_event_v2%'"],
    ["workflow audit kind", "definition not like '%registration_workflow_status_changed%'"],
    ["workflow null finalization compatibility", "definition not like '%''enrollmentFinalization'', null%'"],
    ["workflow finalizer isolation", "definition like '%dashboard_private.finalize_registration_track_enrollments_v1%'"],
    ["workflow notification isolation", "definition like '%dashboard_private.record_registration_management_notification_v1%'"],
    ["workflow observation isolation", "definition like '%registration_observation_%'"],
    ["observation enter delegation", "dashboard_private.enter_registration_observation_v1_impl%"],
    ["observation enter no-op response", "definition not like '%null, null, false%'"],
    ["observation enter stale gate absence", "definition like '%registration_observation_stale_revision%'"],
    ["observation enter event isolation", "definition like '%dashboard_private.write_registration_track_event_v2%'"],
    ["retired notification v1 marker", "registration_workflow_notification_v1_retired%"],
    ["explicit notification manager authorization", "dashboard_private.registration_actor_is_active_manager_v1%"],
    ["explicit notification intent", "p_intent is distinct from ''send_registration_management_notification''%"],
    ["explicit notification UUID request key", "dashboard_private.try_registration_event_uuid(v_request_key)%"],
    ["explicit notification intent SQLSTATE", "registration_management_notification_intent_invalid%using errcode = ''22023''%"],
    ["notification readiness conflict", "registration_management_notification_not_ready%using errcode = ''23514''%"],
    ["notification explicit request", "registration_management_notification_requested%"],
    ["notification materialization", "dashboard_private.record_registration_management_notification_v1%"],
    ["notification archived-track guard", "definition not like '%track.archived_at is null%'"],
    ["notification v2 request ledger", "dashboard_private.notification_request_ledger%"],
    ["notification v2 caller fingerprint", "v_request_fingerprint := pg_catalog.md5%"],
    ["notification v2 caller actor fingerprint", "''actorProfileId'', v_actor%"],
    ["notification v2 request fingerprint", "v_ledger.request_fingerprint <> v_request_fingerprint%"],
    ["notification v2 current-source fence", "dashboard_private.registration_management_notification_source_current_v2%"],
    ["notification v2 fact-scoped lock", "registration-management-notification-v2:%"],
    ["notification v2 bounded semantic dedupe", "with candidate_sources as materialized%"],
    ["notification v2 reusable-source recheck", "v_reusable_source := found%dashboard_private.registration_management_notification_source_current_v2%"],
    ["notification v2 new-source path", "else%v_source_event_id := dashboard_private.write_registration_track_event_v2%"],
    ["notification v2 dedupe receipt", "''alreadyRequested'', v_already_requested%"],
    ["notification v2 stale suppression", "registration_management_notification_snapshot_stale%"],
    ["notification v2 no-send receipt", "''sourceEventIds'', ''[]''::jsonb%"],
    ["notification v2 uncertainty preservation", "external_attempt_uncertainty_preserved%"],
    ["notification request ledger relation", "dashboard_private.notification_request_ledger'::pg_catalog.regclass"],
    ["notification request UUID primary key", "PRIMARY KEY (request_id)"],
    ["notification source original request UUID", "v_request_id := dashboard_private.try_registration_event_uuid%"],
    ["notification source original ledger lookup", "ledger.request_id = v_request_id%"],
    ["notification source original actor fingerprint", "v_ledger.request_fingerprint = pg_catalog.md5%"],
    ["notification source ledger track receipt", "v_ledger.response_payload ->> ''trackId'' = v_track_id::text%"],
    ["notification source ledger revision receipt", "v_ledger.response_payload ->> ''workflowRevision''%v_snapshot ->> ''workflowRevision''%"],
    ["notification source ledger event receipt", "v_ledger.response_payload ->> ''eventKey'' = v_event_key%"],
    ["notification source ledger intent receipt", "v_ledger.response_payload ->> ''intent''%''send_registration_management_notification''%"],
    ["notification source ledger checksum receipt", "v_ledger.response_payload ->> ''factsChecksum'' = v_facts_checksum%"],
    ["notification snapshot task identity", "''taskId''%task.id%"],
    ["notification snapshot track identity", "''trackId''%track.id%"],
    ["notification snapshot task status", "''taskStatus''%task.status%"],
    ["notification snapshot active subjects", "''activeSubjects''%pg_catalog.array_agg(%"],
    ["notification snapshot stable subject order", "dashboard_private.registration_subject_sort_order(%active_track.subject%"],
    ["notification snapshot requester", "''requestedBy''%task.requested_by%"],
    ["notification snapshot director", "''directorProfileId''%track.director_profile_id%"],
    ["notification snapshot normalized memo", "''memo''%nullif(pg_catalog.btrim(coalesce(detail.request_note, '''')), '''')%"],
    ["notification snapshot subject", "''subject''%track.subject%"],
    ["notification snapshot pipeline", "''pipelineStatus''%track.pipeline_status%"],
    ["notification snapshot actor display", "''actorDisplayName''%coalesce(%nullif(actor.name, '''')%"],
    ["notification canonical subjects check", "v_canonical.payload -> ''subjects''%v_snapshot -> ''activeSubjects''%"],
    ["notification canonical requester check", "v_canonical.payload ->> ''requester_profile_id''%v_snapshot ->> ''requestedBy''%"],
    ["notification canonical director check", "v_canonical.payload ->> ''director_profile_id''%v_snapshot ->> ''directorProfileId''%"],
    ["notification canonical memo check", "v_canonical.payload ->> ''memo''%v_snapshot ->> ''memo''%"],
    ["notification canonical actor check", "v_canonical.payload ->> ''actor_name''%v_snapshot ->> ''actorDisplayName''%"],
    ["notification legacy begin final gate", "final_gate.function_key = 'legacy_begin_final_gate'"],
    ["notification external-attempt final gate", "final_gate.function_key = 'external_attempt_final_gate'"],
    ["notification final-gate stale reason", "v_reason := ''registration_management_notification_snapshot_stale''%"],
    ["notification stale plan provider-zero", "like '%''items'', ''[]''::jsonb%'"],
    ["manager write fence trigger", "trigger.tgname = 'enforce_registration_manager_write_fence_v1'"],
    ["manager write fence table family", "pg_catalog.substr(relation.relname, 1, 17) = 'ops_registration_'"],
    ["appointment integrity trigger", "trigger.tgname = 'registration_appointment_integrity_on_track'"],
    ["appointment deterministic SQLSTATE", "registration_invalid_source_state%using errcode = ''23514''%"],
    ["archive synchronous cancellation trigger absent", "trigger.tgname = 'cancel_registration_archived_subject_delivery'"],
    ["archive fact triggers avoid notification tables", "(notification_deliveries|registration_customer_reminder_jobs|ops_registration_customer_messages|registration_customer_message_bundles)"],
    ["archive bundle collector guard", "delivery_guard.function_key = 'bundle_collect'"],
    ...
    ["identity claim gate removed", "definition like '%message.claim_active%'"],
    ["identity batch gate removed", "definition like '%ops_registration_admission_batches%'"],
    ["identity enrollment gate removed", "definition like '%enrollment.status = ''planned''%'"],
    ["common revision nonretryable conflict", "definition not like '%registration_common_revision_conflict'' using errcode = ''23514''%'"],
    ["identity correction gate removed", "definition like '%registration_student_identity_correction_required%'"],
    ["common completeness gate removed", "definition like '%registration_student_name_required%'"],
    ["common reminder runtime removed", "definition like '%assert_registration_reminder_runtime_v1%'"],
    ["common notification control plane removed", "definition like '%notification_control_plane%'"],
    ["common reminder cancel removed", "definition like '%cancel_registration_appointment_reminders_v1%'"],
    ["common reminder materialization removed", "definition like '%materialize_registration_appointment_reminders_v1%'"],
    ["common parent recompute removed", "definition like '%recompute_registration_parent%'"],
    ["subject soft archive event", "definition not like '%registration_subject_archived%'"],
    ["subject restore event", "definition not like '%registration_subject_restored%'"],
    ...
    ["flat create legacy receipt", "definition not like '%create_case_with_initial_workflow_v1%'"],
    ["flat create no completeness", "definition like '%registration_school_grade_required%'"],
    ["flat create title fallback", "definition not like '%coalesce(''등록: '' || v_student_name, ''등록'')%'"],
    ["legacy create empty appointments", "definition not like '%''appointments'', ''[]''::jsonb%'"],
    ["retired reminder create ACL", "dashboard_private.create_registration_case_with_reminders_v1_impl(text,text,text,text,text,text,timestamp with time zone,text[],text,text,jsonb,jsonb,jsonb,jsonb,text)"],
    ["collaboration trigger count", "trigger.tgname = 'enforce_registration_collaboration_write_role_v1'"],
    ["collaboration trigger event coverage", "and trigger.tgtype = 31"],
    ["collaboration RLS insert command", "policy.cmd = ('IN' || 'SERT')"],
    ["collaboration RLS manager check", "registration_observation_current_actor_is_active_manager_v1"],

...
test("calendar main source exposes overflow, quick-add, existing-event drag-drop hooks, annual-board shortcuts, and exam-range hover details", () => {
  ...
  assert.match(source, /placeholder="학교명, 일정명, 유형 검색"/);
  assert.match(source, /onKeyDown=\{\(event\) => \{/);
  assert.match(source, /event\.key === "Escape" && query/);
  assert.match(source, /className=\{cn\("w-full pl-10", query \? "pr-16" : ""\)\}/);
  assert.match(source, /aria-label="학사일정 검색어 지우기"/);
  assert.match(source, /onClick=\{\(\) => setQuery\(""\)\}/);
  ...
  assert.match(source, /didFinishRangeSelectionRef/);
  assert.match(source, /didFinishRangeSelectionRef\.current = true/);
  assert.match(source, /if \(didFinishRangeSelectionRef\.current\) \{/);
  assert.match(source, /didFinishRangeSelectionRef\.current = false/);
  assert.match(source, /const handleDayNumberClick/);
  assert.match(source, /const handleDayNumberClick[\s\S]*didFinishRangeSelectionRef\.current = false/);
  assert.match(source, /const handleDayCellClick/);
  assert.match(source, /onClick=\{\(\) => handleDayCellClick\(day, dayEvents\.length > 0\)\}/);
  assert.match(source, /if \(!readOnly && !hasEvents\) \{/);

  assert.match(source, /onEmptySlotClick\?\.\(day\)/);
  assert.match(source, /onDateSelect\?\.\(day\)/);
...
test("event form source uses exam-focused fields and compact row layout", () => {
  ...
  assert.match(source, /const showExamTermField = isExamTypeWithTerm\(formData\.typeLabel\)/);
  assert.match(source, /const showScopeFields = isSubjectExamType\(formData\.typeLabel\)/);
  assert.match(source, /cn\("grid gap-4 md:grid-cols-2", showExamTermField && "xl:grid-cols-\[minmax\(0,1\.6fr\)_minmax\(0,1fr\)_minmax\(0,1fr\)\]"\)/);
  assert.match(source, /<Label htmlFor="title">일정 제목<\/Label>/);
  assert.match(source, /placeholder="일정 제목 입력"/);
  assert.match(source, /<Label>일정 유형<\/Label>/);
  assert.match(source, /<Label>학년<\/Label>/);
  assert.match(source, /<Label>학교<\/Label>/);
  assert.match(source, /<Label htmlFor="date">시작일<\/Label>/);
  assert.match(source, /<Label htmlFor="end-date">종료일<\/Label>/);
  assert.match(source, /메모 \(선택\)/);
  ...
  assert.doesNotMatch(source, /CalendarIcon/);
  assert.doesNotMatch(source, /GraduationCap/);
  assert.doesNotMatch(source, /School className/);
  assert.doesNotMatch(source, /Tag className/);
  assert.doesNotMatch(source, /Type className/);

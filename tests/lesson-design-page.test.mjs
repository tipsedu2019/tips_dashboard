...
test("lesson design keeps every generated month visible while focusing one month", async () => {
  ...
  assert.match(source, /2xl:sticky 2xl:top-20/);
  assert.match(source, /2xl:max-h-\[calc\(100vh-6rem\)\]/);
...
test("lesson design ranks class-fit textbooks and presets session ranges", async () => {
  const source = await readSource("src/features/operations/class-schedule-workspace.tsx");
  ...
  assert.match(source, /scoreLessonTextbookCandidate\(right, lessonDesignSnapshot\)/);
  assert.match(source, /buildLessonOutlinePresets/);
  assert.match(source, /buildDerivedLessonRangePresets/);
  assert.match(source, /handleLessonTextbookPlanPreset/);
  assert.match(source, /handleLessonTextbookPlanAutoFill/);
  assert.match(source, /handleLessonTextbookPlanAutoFillAll/);
  assert.match(source, /aria-pressed=\{entry\.planLabel === preset\.label\}/);
  ...
  assert.match(source, /applyLessonTextbookPlanRangeFields/);
  assert.match(source, /lessonTextbookProgressSessions\s*\.slice\(startIndex\)/);
  ...
  assert.match(source, /목차 프리셋/);
  assert.match(source, /기간에 포함/);
  assert.match(source, /전체 기간/);
  assert.match(source, /현재 회차부터/);
  assert.match(source, /현재 회차 자동 배정/);
  assert.match(source, /이후 회차 자동 배정/);
  ...
  assert.match(source, /title=\{preset\.label\}/);
  assert.match(source, /preset\.label\.startsWith\(`\$\{entry\.scopeLabel\} `\)/);
...
test("lesson design separates textbook finder and connected textbook ranges", async () => {
  ...
  assert.doesNotMatch(source, /session\.textbookEntries\.map\(\(entry\) =>/);
...
test("lesson design presents compact filters, selected rows, and mobile-safe sessions", async () => {
  ...
  assert.match(source, /auto-fill-current-session/);
  assert.match(source, /auto-fill-following-sessions/);
});

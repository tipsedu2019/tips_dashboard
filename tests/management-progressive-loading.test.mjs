...
test("management hook and UI keep list reads bounded while details and relation pickers are selection driven", async () => {
  ...
  assert.match(activeHook, /readService\.loadInitialPage\(\{[\s\S]*?kind,[\s\S]*?filters,[\s\S]*?cursor: null,[\s\S]*?limit: pageSize,[\s\S]*?canonicalReplayToken,[\s\S]*?coalesceInitialRequest: allowCanonicalReplay,[\s\S]*?signal: ticket\.signal/);
  assert.match(activeHook, /setRows\(normalizeManagementRows\(kind, sourceRows\)\)[\s\S]*?setLoading\(false\)[\s\S]*?await result\.metadata/);
  assert.match(activeHook, /metadata\.ok[\s\S]*?aggregateToStats\(kind, metadata\.stats\)[\s\S]*?setFilterOptions\(metadata\.filterOptions\)/);
  ...
  assert.match(activeHook, /readService\.loadNextPage\(\{[\s\S]*?kind,[\s\S]*?filters: effectiveFiltersRef\.current,[\s\S]*?cursor: nextCursor,[\s\S]*?limit: pageSize,[\s\S]*?signal: ticket\.signal/);

...
test("브라우저 검증은 실제 달력 항목으로 같은 신청서를 열고 예약 섹션에 초점을 둔다", async () => {
  ...
  assert.match(verifier, /calendarItem\.click\(\)/);
  assert.match(verifier, /calendar focus/);
  assert.match(verifier, /registration-application-level_test/);
  assert.match(verifier, /data-registration-appointment-focus/);
  assert.match(verifier, /data-registration-appointment-shared-controls/);
  assert.match(verifier, /data-registration-appointment-subjects="영어\|수학"/);
  assert.doesNotMatch(verifier, /예약 저장[^\n]*click\(/);
...
test("pgTAP 달력 계약 수와 계획 수가 일치한다", async () => {
  ...
  const assertionCount = (
    source.match(
      /^select\s+(?:ok|is|isnt|is_empty|isnt_empty|lives_ok|throws_ok|results_eq|set_eq|bag_eq|col_is_null|col_not_null)\s*\(/gim,
    ) || []
  ).length;

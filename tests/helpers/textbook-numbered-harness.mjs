...
export const purchaseSummary = (mode = 'request', totalCount = 1) => {
  ...
  const groups = totalCount > 0 ? [{ status: 'requested', totalCount, rawLineCount: 2 * totalCount, quantities }] : [];
  return { mode, totalCount, rawLineCount: 2 * totalCount, quantities, groups, requestCounts: { all: totalCount, unregistered: 0, orderable: totalCount }, orderCounts: { all: totalCount, waiting: totalCount, partial: 0, returnable: 0, returned: 0 }, boardScopeCounts: { all: totalCount, active: totalCount, recent: totalCount } };
};
...
export const saleSummary = (totalCount = 1) => ({ totalCount, totalQuantity: 2 * totalCount, studentCount: 0, classCount: totalCount > 0 ? 1 : 0, totalAmount: 20000 * totalCount, groups: totalCount > 0 ? [{ status: 'charged', totalCount, totalQuantity: 2 * totalCount }] : [], statusCounts: { all: totalCount, waiting: totalCount, issued: 0, returned: 0, cancelled: 0 } });
...
function modules(supabase, overrides) {
  ...
  function load(file) {
    ...
    const exposePrivateTestComponents = file.endsWith('textbook-operations-workspace.tsx')
      ? '\nmodule.exports.__testOnlyTeacherSelect = TeacherSelect; module.exports.__testOnlyHandoffDialog = TextbookHandoffDialog;'
      : '';
...
export async function setup(t, initial = {}) {
  ...
  for (const [key, value] of Object.entries(initial.localStorage || {})) {
  ...
  const testComponentCleanups = new Set();
  t.after(async () => {
    await unmount();
    for (const cleanup of testComponentCleanups) await cleanup();
    dom.window.close();
  });
  await render();
  const mountTestComponent = async (Component, props) => {
    ...
    const cleanup = async () => {
      ...
      node.remove();
      testComponentCleanups.delete(cleanup);
    };

...
`)
    ...
    const discardConfirmation = page.getByRole("dialog", { name: "저장하지 않은 변경사항이 있습니다", exact: true })
    await discardConfirmation.getByRole("button", { name: "변경사항 버리기", exact: true }).click()
    assert.equal(await ruleSwitch().isChecked(), true, "discarding only the template preserves the staged rule")
    assert.equal(await mentionSwitch().isChecked(), true, "discarding only the template preserves the staged mention")
    await openEditor()

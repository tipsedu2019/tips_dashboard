...
-- Local isolated DB fixture only. The reviewed baseline omits these 20260803
-- function definitions and contract rows while retaining their migration ledger.
-- Restore exact original definitions after baseline parity, before new migrations.
-- source: supabase/migrations/20260803140000_notification_content_contracts.sql
-- source-sha256: c501226c91e88ac92b4464847f026f4950195d5f8333e66b682e3395fae06280
-- source: supabase/migrations/20260803150000_notification_makeup_content_single_writer.sql
-- source-sha256: 27d9bd79cab12492a8955354c69894829edda79376a16242152a0e6aeeb54183

do $guard$
begin
  if pg_catalog.to_regprocedure('public.save_notification_control_plane_with_override_v2(text,jsonb,jsonb,jsonb,uuid,uuid,jsonb)') is not null
    or pg_catalog.to_regprocedure('public.save_notification_control_plane_v2(text,jsonb,jsonb,jsonb,uuid)') is not null
    or pg_catalog.to_regprocedure('dashboard_private.mirror_makeup_notification_template_v1(uuid,uuid,uuid)') is not null
    or exists(select 1 from dashboard_private.notification_rule_content_contracts) then
    raise exception 'isolated_notification_settings_fixture_state_invalid' using errcode = '55000';
  end if;
end;
$guard$;
...
alter function dashboard_private.notification_content_event_spec_v1(text) owner to postgres;
revoke all on function dashboard_private.notification_content_event_spec_v1(text) from public, anon, authenticated, service_role;
alter function dashboard_private.notification_content_variable_v1(text) owner to postgres;
revoke all on function dashboard_private.notification_content_variable_v1(text) from public, anon, authenticated, service_role;
alter function dashboard_private.notification_content_contract_for_identity_v1(text,text,text) owner to postgres;
revoke all on function dashboard_private.notification_content_contract_for_identity_v1(text,text,text) from public, anon, authenticated, service_role;
alter function dashboard_private.notification_content_contract_for_rule_v1(uuid) owner to postgres;
revoke all on function dashboard_private.notification_content_contract_for_rule_v1(uuid) from public, anon, authenticated, service_role;
alter function dashboard_private.notification_template_compliance_v1(uuid,uuid) owner to postgres;
revoke all on function dashboard_private.notification_template_compliance_v1(uuid,uuid) from public, anon, authenticated, service_role;
alter function public.save_notification_control_plane_with_override_v2(text,jsonb,jsonb,jsonb,uuid,uuid,jsonb) owner to postgres;
revoke all on function public.save_notification_control_plane_with_override_v2(text,jsonb,jsonb,jsonb,uuid,uuid,jsonb) from public, anon, authenticated, service_role;
alter function dashboard_private.mirror_makeup_notification_template_v1(uuid,uuid,uuid) owner to postgres;
revoke all on function dashboard_private.mirror_makeup_notification_template_v1(uuid,uuid,uuid) from public, anon, authenticated, service_role;
alter function dashboard_private.save_notification_control_plane_unmirrored_v2(text,jsonb,jsonb,jsonb,uuid) owner to postgres;
revoke all on function dashboard_private.save_notification_control_plane_unmirrored_v2(text,jsonb,jsonb,jsonb,uuid) from public, anon, authenticated, service_role;
alter function public.save_notification_control_plane_v2(text,jsonb,jsonb,jsonb,uuid) owner to postgres;

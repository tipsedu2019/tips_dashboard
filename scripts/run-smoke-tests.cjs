...
const compiledRoot = path.join(
  os.tmpdir(),
  `tips-smoke-compiled-${process.pid}`,
);
...
const { AuthProvider } = require(
  path.join(compiledSourceRoot, "contexts", "AuthContext.jsx"),
);
const { ToastProvider } = require(
  path.join(compiledSourceRoot, "contexts", "ToastContext.jsx"),
);
const App = require(path.join(compiledSourceRoot, "App.jsx")).default;
const DataManager = require(
  path.join(compiledSourceRoot, "components", "DataManager.jsx"),
).default;
const PublicClassListView = require(
  path.join(compiledSourceRoot, "components", "PublicClassLandingView.jsx"),
).default;
const ClassListWorkspace = require(
  path.join(compiledSourceRoot, "components", "ClassListWorkspace.jsx"),
).default;
const AcademicCalendarView = require(
  path.join(compiledSourceRoot, "components", "AcademicCalendarView.jsx"),
).default;
const CurriculumRoadmapView = require(
  path.join(compiledSourceRoot, "components", "CurriculumRoadmapView.jsx"),
).default;
const TeacherWeeklyView = require(
  path.join(compiledSourceRoot, "components", "TeacherWeeklyView.jsx"),
).default;
const BottomSheet = require(
  path.join(compiledSourceRoot, "components", "ui", "BottomSheet.jsx"),
).default;
const TimetableGrid = require(
  path.join(compiledSourceRoot, "components", "ui", "TimetableGrid.jsx"),
).default;
const ClassEditor = require(
  path.join(
    compiledSourceRoot,
    "components",
    "data-manager",
...
const { createE2EMockData } = require(
  path.join(compiledSourceRoot, "testing", "e2e", "mockAppData.js"),
);
const { e2eDataService } = require(
  path.join(compiledSourceRoot, "testing", "e2e", "mockDataService.js"),
);
const { ACTIVE_CLASS_STATUS } = require(
  path.join(compiledSourceRoot, "lib", "classStatus.js"),
);
const { buildSchedulePlanForSave } = require(
  path.join(compiledSourceRoot, "lib", "classSchedulePlanner.js"),
);
...
async function renderCurriculumRoadmap(width) {
  ...
  await waitFor(() => {
    if (width <= 768) {
      assert.ok(screen.getByTestId("roadmap-top-rail"));
      return;
    }
...
async function main() {
  ...
  await runTest(
    "public mobile renders landing shell without condensed header resizing",
    async () => {
      setViewport(390, 844);
      ...
      await waitFor(() => {
        assert.ok(
          !document.querySelector(".public-landing-shell.is-condensed-header"),
        );
      });
    },
  );
  ...
  await runTest(
    "public mobile subject and grade tabs filter class cards",
    async () => {
  ...
  await runTest(
    "public mobile review and score tabs open new windows and keep the class list visible",
    async () => {
      const user = userEvent.setup();
      setViewport(390, 844);
      ...
      try {
        ...
        assert.match(openCalls[0][0], /placePath=%2Freview/);
        assert.match(openCalls[1][0], /tipsedu\.notion\.site/);
        assert.ok(screen.getByTestId("public-card-list"));
        assert.equal(
          document.querySelector(".public-bottom-nav-button.is-active span")
            ?.textContent,
          "수업",
        );
      } finally {
  ...
  await runTest(
    "public mobile planner blocks conflicts and opens planner sheet",
    async () => {
      const user = userEvent.setup();
      setViewport(390, 844);
      ...
      assert.equal(
        document.querySelectorAll(".public-planner-selected-item").length,
        1,
      );
    },
  );
  ...
  await runTest(
    "public mobile search preserves the active grade after clearing empty results",
    async () => {
      const user = userEvent.setup();
      setViewport(390, 844);
      ...
      await waitFor(() => {
        assert.ok(
          document.querySelector(
            ".public-landing-tab-row-grade .public-landing-pill-tab.is-active",
          ),
        );
      });

      const initialGradeLabel = document
        .querySelector(
          ".public-landing-tab-row-grade .public-landing-pill-tab.is-active",
        )
        ?.textContent?.trim();
  ...
  await runTest(
    "editable timetable grid keeps touch swipe scrolling available",
    async () => {
      setViewport(390, 844);
      ...
      const { container } = render(
        React.createElement(TimetableGrid, {
          ...
          timeSlots: ["09:00-09:30", "09:30-10:00", "10:00-10:30"],
          blocks: [],
          editable: true,
          editableMode: "edit",
          slotHeight: 48,
      ...
      const shell = container.querySelector(".timetable-grid-shell");
      assert.ok(shell);
      assert.equal(shell.style.touchAction, "pan-y pinch-zoom");
      ...
      assert.ok(shell.scrollLeft > 0);
    },
  );

  await runTest(
    "stacked bottom sheets keep the last sheet in control of escape and scroll lock",
    async () => {
      setViewport(390, 844);

      const closeCalls = [];
      const handleFirstClose = () => closeCalls.push("first");
      const handleSecondClose = () => closeCalls.push("second");
  ...
  await runTest(
    "mobile academic views switch from the bottom nav without a duplicate top switcher",
    async () => {
      const user = userEvent.setup();
      ...
      await waitFor(() => {
        assert.equal(screen.queryByTestId("mobile-academic-switcher"), null);
        assert.equal(screen.queryByTestId("app-bottom-nav-logout"), null);
        assert.ok(screen.getByTestId("dashboard-floating-actions"));
      });

      await user.click(screen.getByTestId("mobile-nav-curriculum-roadmap"));
  ...
  await runTest(
    "mobile timetable filter sheet includes unified term filter",
    async () => {
      const user = userEvent.setup();
      await renderAppMobile("staff");
  ...
  await runTest(
    "desktop timetable unified filters narrow teacher and classroom options by subject",
    async () => {
      const user = userEvent.setup();
      ...
      const subjectButtons = document.querySelectorAll(
        ".timetable-unified-filter-section-subject .timetable-unified-chip",
      );
      assert.ok(subjectButtons.length >= 2);
      assert.ok(
        document.querySelectorAll(
          ".timetable-unified-filter-section-teacher .timetable-unified-chip",
        ).length >= 2,
      );
      assert.ok(
        document.querySelectorAll(
          ".timetable-unified-filter-section-classroom .timetable-unified-chip",
        ).length >= 2,
      );
  ...
  await runTest(
    "class list mobile card opens the schedule plan sheet",
    async () => {
      const user = userEvent.setup();
      await renderAppMobile("staff");
  ...
  await runTest(
    "class list quick filters narrow teacher and classroom options",
    async () => {
      const user = userEvent.setup();
      ...
      await waitFor(() => {
        assert.equal(
          screen.getAllByTestId("quick-filter-option-subject").length,
          2,
        );
        assert.equal(
          screen.getAllByTestId("quick-filter-option-teacher").length,
          2,
        );
        assert.equal(
          screen.getAllByTestId("quick-filter-option-classroom").length,
          2,
        );
      });
  ...
  await runTest(
    "class list mobile advanced filter sheet reveals overflow filters",
    async () => {
      const user = userEvent.setup();
  ...
  await runTest(
    "class manager uses unified filter panel and keeps desktop controls",
    async () => {
      const user = userEvent.setup();
      await renderDataManager(1440);

      const tabButtons = Array.from(
        document.querySelectorAll(".h-segment-container .h-segment-btn"),
      );
  ...
  await runTest(
    "academic calendar mobile filter and create sheets open",
    async () => {
      const user = userEvent.setup();
      await renderAcademicCalendar(390);

      await user.click(screen.getByTestId("calendar-top-rail-filter-button"));
      ...
      await user.click(screen.getByTestId("calendar-top-rail-add-button"));
      ...
      await user.selectOptions(
        screen.getByTestId("academic-editor-category-select"),
        "middle",
      );
  ...
  await runTest(
    "academic calendar mobile mode toggle swaps month and agenda",
    async () => {
      const user = userEvent.setup();
      await renderAcademicCalendar(390);
  ...
  await runTest(
    "curriculum roadmap mobile filter sheet and editor open",
    async () => {
      const user = userEvent.setup();
      await renderCurriculumRoadmap(390);

      await user.click(screen.getByTestId("roadmap-top-rail-filter-button"));
      ...
      const roadmapCells = screen.getAllByTestId(
        /roadmap-school-cell-|roadmap-school-board-cell-|roadmap-academy-cell-/,
      );
      ...
      const scheduleTypeSelect = screen.getByTestId(
        "roadmap-schedule-type-select",
      );
      assert.equal(scheduleTypeSelect.value, "영어시험일");
      const scheduleTypeOptions = Array.from(scheduleTypeSelect.options).map(
        (option) => option.value,
      );
  ...
  await runTest(
    "teacher weekly mobile agenda shows compact axis picker",
    async () => {
      const user = userEvent.setup();
      ...
      const dayTabs = screen.getAllByRole("button", {
        name: /월|화|수|목|금|토|일/,
      });
  ...
  await runTest(
    "academic calendar roadmap action supports schedule-column event types",
    async () => {
      const user = userEvent.setup();
      const intents = [];
      const data = createE2EMockData();
  ...
  await runTest(
    "curriculum roadmap school tab uses annual board only and supports reverse academic selection",
    async () => {
      const user = userEvent.setup();
      const intents = [];
      const data = createE2EMockData();
      ...
      await user.click(
        screen.getByTestId(
          "roadmap-calendar-link-schedule-school-1-field-trip",
        ),
      );
      ...
      await user.click(
        screen.getByTestId("roadmap-calendar-event-option-event-field-trip-2"),
      );
  ...
  await runTest(
    "academic calendar navigation intent preserves existing filters",
    async () => {
      const user = userEvent.setup();
      const data = createE2EMockData();
      enableE2EMode("staff");
      ...
      assert.equal(
        screen
          .getByTestId("calendar-filter-type-방학·휴일")
          .getAttribute("aria-pressed"),
        "true",
      );
      assert.equal(
        screen
          .getByTestId("calendar-filter-type-체험학습")
          .getAttribute("aria-pressed"),
        "false",
      );
          ...
          React.createElement(
            ...
            React.createElement(AcademicCalendarView, {
              ...
              navigationIntent: {
                ...
                eventType: "체험학습",
                date: data.academicEvents.find(
                  (event) => event.id === "event-field-trip",
                )?.start,
              },
            }),
          ),
  ...
  await runTest(
    "curriculum roadmap navigation intent preserves existing filters",
    async () => {
      const user = userEvent.setup();
      const data = createE2EMockData();
      enableE2EMode("staff");
      ...
      await user.selectOptions(
        screen.getByTestId("roadmap-filter-subject-select"),
        "수학",
      );
      assert.equal(
        screen.getByTestId("roadmap-filter-subject-select").value,
        "수학",
      );
  ...
  await runTest("overview ignores timetable panel selections", async () => {
    ...
    await user.click(screen.getByTestId("mobile-nav-stats"));

    await waitFor(() => {
      assert.equal(
        screen.getByTestId("stats-total-classes").textContent.trim(),
        "2개",
      );
    });
  });
  ...
  await runTest(
    "class editor shows live schedule conflicts while typing",
    async () => {
      const user = userEvent.setup();
      ...
      await user.type(
        screen.getByTestId("class-editor-schedule-input"),
        "월 16:00-17:30",
      );
  ...
  await runTest(
    "class editor plan panel uses the shared class plan branding",
    async () => {

...
with migration_ledger as (
  ...
  select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
    ...
    'definitionSha256', dashboard_private.continuous_class_schedule_hash_v1(scoped_catalog.fingerprint)
...
export function normalizeDashboardFunctionIdentity(value) {
  ...
  const argumentsList = value.slice(opening + 1, -1).trim().replaceAll(/\s*,\s*/gu, ",");
...
export async function captureDashboardFreeTierCatalog({ argv = process.argv.slice(2), env = process.env, root = ROOT, fetch = globalThis.fetch, gitOriginMainSha = async () => (await import("node:child_process")).execFileSync("git", ["rev-parse", "origin/main"], { cwd: root, encoding: "utf8" }).trim(), log = () => {}, afterStageRename = async () => {}, publish = async ({ stage, final, active, captureId, artifactPaths, artifactTargets, artifacts }) => {
  ...
  try {
    ...
    for (let index = 0; index < outputs.length; index += 1) await rename(temporaryOutputs[index], outputs[index][0]);
    await writeFile(pointer, `${JSON.stringify({ captureId, artifactPaths })}\n`, { mode: 0o600 });
    await rename(pointer, active);
  } catch (error) {
    if (previousOutputs) {
      for (let index = 0; index < outputs.length; index += 1) {
        ...
        try {
          if (previous.kind === "file") {
            ...
            await rename(restorationOutputs[index], target);

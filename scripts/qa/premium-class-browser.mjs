...
for(const width of [1440,390]) for(const theme of ['light']) {
 ...
 await page.screenshot({path:`${out}/class-list-${width}.png`});await first.click();const dialog=page.getByRole('dialog');await dialog.getByLabel('수업명',{exact:true}).waitFor();await page.evaluate(()=>document.fonts.ready);
 assert.equal(await dialog.getByLabel('수업명',{exact:true}).inputValue(),className);await page.screenshot({path:`${out}/class-detail-${width}.png`});row.detailOverflow=await page.evaluate(()=>Math.max(0,document.documentElement.scrollWidth-innerWidth));assert.equal(row.detailOverflow,0);
 ...
 await roster.getByTestId('class-roster-student-name-link').click();await page.getByRole('button',{name:'취소',exact:true}).click();await dialog.getByLabel('수업명',{exact:true}).scrollIntoViewIfNeeded();await dialog.getByLabel('수업명',{exact:true}).waitFor();
 await dialog.getByLabel('수업명',{exact:true}).fill('수정한 합성 수업명');await page.getByRole('button',{name:'수업 상세 닫기',exact:true}).click();await page.getByRole('button',{name:'계속 편집',exact:true}).click();assert.equal(await dialog.getByLabel('수업명',{exact:true}).inputValue(),'수정한 합성 수업명');

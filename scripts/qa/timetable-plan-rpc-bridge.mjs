...
export async function planFixtureRpc(name,args){
 ...
 const sql=`begin; set local role authenticated; set local request.jwt.claim.sub = ${quote(actor)}; set local request.jwt.claims = ${quote(JSON.stringify({sub:actor,role:'authenticated'}))}; select public.${name}(${parameters})::text; commit;`;

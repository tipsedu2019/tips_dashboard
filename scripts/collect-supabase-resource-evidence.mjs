...
const sectionStatements = {
  database: statement("select pg_catalog.clock_timestamp() as captured_at, pg_catalog.pg_postmaster_start_time() as postmaster_started_at, pg_catalog.pg_database_size(pg_catalog.current_database()) as database_bytes, d.numbackends, d.xact_commit, d.xact_rollback, d.temp_files, d.temp_bytes, d.deadlocks from pg_catalog.pg_stat_database d where d.datname = pg_catalog.current_database() limit 1"),
  ...
  activity_blockers: statement("select a.state, a.wait_event_type, pg_catalog.count(*)::bigint as session_count, pg_catalog.count(*) filter (where pg_catalog.cardinality(pg_catalog.pg_blocking_pids(a.pid)) > 0)::bigint as blocked_session_count, pg_catalog.max(pg_catalog.clock_timestamp() - a.query_start) filter (where a.state = 'active') as max_active_query_age, pg_catalog.max(pg_catalog.clock_timestamp() - a.xact_start) filter (where a.xact_start is not null) as max_transaction_age from pg_catalog.pg_stat_activity a group by a.state, a.wait_event_type order by session_count desc limit 20"),
  statements: statement("select s.queryid::text, s.calls, s.total_exec_time, s.shared_blks_read, s.temp_blks_read, s.temp_blks_written, encode(extensions.digest(s.query, 'sha256'), 'hex') as statement_sha256 from extensions.pg_stat_statements s order by s.total_exec_time desc, s.shared_blks_read desc limit 20"),

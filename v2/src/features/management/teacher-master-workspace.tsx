...
function reorderWithSequentialSort(rows: TeacherRecord[], fromIndex: number, toIndex: number) {
  const nextRows = [...rows];
...
export function TeacherMasterWorkspace() {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "선생님 정보를 저장하지 못했습니다.");
    } finally {
      setSaving(false);
    }
  };
                  ...
                  <TableRow key={row.id}>
                    ...
                    {isColumnVisible("action") ? <TableCell className={settingsTableCellClass}>
                      <div className="flex justify-end gap-2">
                        ...
                        <Button type="button" variant="outline" size="icon" className="size-8" onClick={() => handleMoveRow(row.id, "down")} disabled={saving || currentIndex === rows.length - 1} aria-label="선생님 순서 아래로 이동">
                          <ArrowDown className="size-4" />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" className="size-8 text-destructive hover:text-destructive" onClick={() => handleDelete(row)} disabled={saving} aria-label="선생님 삭제">

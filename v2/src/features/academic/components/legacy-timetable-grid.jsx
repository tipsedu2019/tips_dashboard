...
function TimetableGrid({
  ...
  return (
    <div
      ref={shellRef}
      className={['timetable-grid-shell', `is-${density}`, fitColumns ? 'is-fit-columns' : '', shellClassName].filter(Boolean).join(' ')}
      style={{
        overflowX: fitColumns ? 'hidden' : density === 'micro' || density === 'nano' ? 'hidden' : 'auto',
        minWidth: 0,

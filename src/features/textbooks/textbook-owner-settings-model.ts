...
function operation(value: unknown): asserts value is OwnerDraftOperation {
  ...
  switch (row.type) {
    case "publisher.add": if (!exact(value, ["type", "id", "name", "subjects", "supplierIds"]) || typeof row.name !== "string" || !strings(row.subjects) || !strings(row.supplierIds) || !row.supplierIds.every(uuid) || new Set(row.supplierIds.map((id: string) => id.toLowerCase())).size !== row.supplierIds.length) fail(); break;
    case "publisher.patch": if (!exact(value, ["type", "id", "patch"]) || !object(row.patch)) fail(); fields(row.patch, ["name", "subjects", "supplierIds"], true); if ((own(row.patch, "name") && typeof row.patch.name !== "string") || (own(row.patch, "subjects") && !strings(row.patch.subjects)) || (own(row.patch, "supplierIds") && (!strings(row.patch.supplierIds) || !row.patch.supplierIds.every(uuid) || new Set(row.patch.supplierIds.map((id: string) => id.toLowerCase())).size !== row.patch.supplierIds.length))) fail(); break;
    case "publisher.delete": if (!exact(value, ["type", "id"])) fail(); break;
    case "supplier.add": if (!exact(value, ["type", "id", "name", "contact", "memo"]) || ![row.name, row.contact, row.memo].every(item => typeof item === "string")) fail(); break;
    case "supplier.patch": if (!exact(value, ["type", "id", "patch"]) || !object(row.patch)) fail(); fields(row.patch, ["name", "contact", "memo"], true); if ((own(row.patch, "name") && typeof row.patch.name !== "string") || (own(row.patch, "contact") && typeof row.patch.contact !== "string") || (own(row.patch, "memo") && typeof row.patch.memo !== "string")) fail(); break;

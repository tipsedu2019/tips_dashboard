...
export function TimetablePlanWorkspace({ state, view, onViewChange, onEditorDirty, onTransferDirty, requestAction, onTransferred }: {
    ...
    const [subject, setSubject] = useState(''), [targets, setTargets] = useState<Record<string, string[]>>({}), [gridCount, setGridCount] = useState(2);
    ...
    const [editor, setEditor] = useState<PlacementEditorDraft | null>(null), [detail, setDetail] = useState<PlanGridBlock | null>(null), [deleteId, setDeleteId] = useState<string | null>(null), [error, setError] = useState(''), [exporting, setExporting] = useState('');
    ...
    const visibleStartMinute = 540, visibleEndMinute = 1440;

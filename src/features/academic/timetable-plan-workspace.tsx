...
import type { GridTarget, PlanItemDraft, TimetableView } from './timetable-plan-contract';
...
export function TimetablePlanWorkspace({ state, view, onViewChange, onFormDirty, requestAction }: {
    ...
    const canEdit = !!snapshot?.permissions.canEdit && snapshot.plan.state === 'draft' && state.referenceStatus === 'verified';
    ...
    const closeEditor = (saved = false) => { const close = () => { setEditor(null); onFormDirty(false); }; if (saved)
        close();
    else
        requestAction(close); };
 ...
 <Dialog open={!!deleteId} onOpenChange={open => { if (!open)
        setDeleteId(null); }}><DialogContent restoreFocusToOpener><DialogHeader><DialogTitle>프리셋 수업 삭제</DialogTitle><DialogDescription>이 수업과 모든 배치를 삭제합니다. 운영 수업은 변경되지 않습니다.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setDeleteId(null)}>취소</Button><Button variant="destructive" disabled={!canEdit} onClick={() => { if (deleteId)

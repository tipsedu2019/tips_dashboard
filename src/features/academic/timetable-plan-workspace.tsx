...
export function TimetablePlanWorkspace({ state, view, onViewChange, onFormDirty, requestAction, onTransferred }: {
 ...
 <TimetableTransferDialog key={state.service?.actorScope} open={transferOpen} onOpenChange={setTransferOpen} state={state} selectedItemIds={selectedItemIds} setSelectedItemIds={setSelectedItemIds} onPending={pending => { setTransferPending(pending); onFormDirty(pending); }} onTransferred={onTransferred}/>
 </WorkspaceTabs>;
}

                ...
                <form className="space-y-4" onSubmit={e => { e.preventDefault(); void submit(); }}>
                    ...
                    {!command ? <>
                        ...
                        <div className="max-h-64 space-y-2 overflow-y-auto">
                            {source.kind === 'preparation' ? sources?.preparationClasses.map(c => <label key={c.id} className="flex items-center gap-2 text-sm"><Checkbox disabled={busy} checked={source.classIds.includes(c.id)} onCheckedChange={checked => choose({ kind: 'preparation', classIds: checked ? [...source.classIds, c.id] : source.classIds.filter(id => id !== c.id) })} />{c.name} · {c.subject}</label>) : sources?.legacyCandidates.map((c, index) => <label key={c.key} className="flex items-center gap-2 text-sm"><input type="radio" name="legacy-source" disabled={busy || c.parseStatus !== 'ready' || !c.entryCount} checked={source.key === c.key} onChange={() => choose({ kind: 'legacy', key: c.key })} /><span className="break-all">{formatLegacyImportLabel(c.key, index)} · {c.entryCount}개 · {c.parseStatus === 'ready' ? c.updatedAt.slice(0, 10) : '형식 확인 필요'}</span></label>)}

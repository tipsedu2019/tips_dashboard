...
type RouteDependencies = Readonly<{
  ...
  listCurrentObservationHistory(input: Readonly<{
    taskId: string
      ...
      try {
        ...
        if (observationTarget) {
          ...
          [readinessValue, historyValue] = await Promise.all([
            dependencies.getReadiness(readinessInput),
...
async function sourceRpc(context: HandlerAuthContext, args: JsonRecord) {
  ...
  if (result.error) {
    console.error(
      `registration_customer_message_source_rpc_failed code=${text(result.error.code)} message=${text(result.error.message)} kind=${args.p_message_kind} source=${args.p_source_id} actor=${args.p_actor_profile_id}`,
    )
...
async function currentObservationHistory(
  context: HandlerAuthContext,
  input: Readonly<{
    taskId: string
    messageKind: "observation_booking" | "observation_reminder"
...
export function createProductionRegistrationCustomerMessageRouteHandlers(
  ...
  return createRegistrationCustomerMessageRouteHandlers({
    ...
    listCurrentObservationHistory(input) {
      return currentObservationHistory(input.context, input)
    },

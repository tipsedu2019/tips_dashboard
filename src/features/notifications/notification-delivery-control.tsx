...
export function GoogleChatDeliveryControl({ eventId, onWarning, allowRetry = true }: { eventId: string | null; onWarning: (message: string) => void; allowRetry?: boolean }) {
  ...
  return <div className="flex flex-wrap items-center gap-2 text-xs">
    ...
    {allowRetry && status.status === "unknown" ? <label className="flex items-center gap-2"><Checkbox checked={confirmedAbsent} onCheckedChange={(value) => setConfirmedAbsent(value === true)} />Google Chat 방에 메시지가 없음을 확인했습니다</label> : null}
    {allowRetry && status.retryAllowed ? <Button type="button" size="sm" variant="outline" disabled={busy || (status.confirmationRequired && !confirmedAbsent)} onClick={async () => {
      setBusy(true)

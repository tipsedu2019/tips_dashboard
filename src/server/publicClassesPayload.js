...
export async function loadPublicClassesEnv(env = process.env) {
  await Promise.all([
    importEnvFile(path.join(projectRoot, ".env"), env),
    importEnvFile(path.join(projectRoot, ".env.local"), env),
    importEnvFile(path.join(projectRoot, ".env.supabase.local"), env),
  ]);
}

...
export function DataTableHeaderRow({ className, children, ...props }: React.ComponentProps<"tr">) {
  return (
    <TableRow className={cn("h-[var(--table-header-height)] border-b border-border/80 hover:bg-transparent", className)} {...props}>
      {children}
    </TableRow>
  );
}
...
export function DataTableBodyRow({ className, children, ...props }: React.ComponentProps<"tr">) {
  return (
    <TableRow
      className={cn(
        "group/data-table-row h-[var(--table-row-height)] border-b border-border/70 transition-colors duration-[var(--motion-duration-control)] ease-[var(--motion-easing-control)] hover:bg-muted data-[state=selected]:bg-accent motion-reduce:transition-none last:border-b-0",
        className,
      )}
      {...props}
    >

...
function ToggleChip({ active, children, onClick, tone = 'default', compact = false, fullWidth = false }) {
  const color = tone === 'danger' ? '#b91c1c' : tone === 'warning' ? '#b45309' : 'var(--accent-color)';
  const background = tone === 'danger' ? 'rgba(239, 68, 68, 0.12)' : tone === 'warning' ? 'rgba(245, 158, 11, 0.14)' : 'var(--accent-light)';
...
function updateSessionStateDraft(existing, { nextState, memo, makeupDate, isForced }) {
  const current = existing || { state: isForced ? 'force_active' : 'active', memo: '', makeupDate: '' };
...
function SectionLabel({ children, tooltip }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-muted)' }}>{children}</div>
...
export default function ClassSchedulePlanner({
  ...
  const overlapIdSet = useMemo(() => new Set(calculation.overlapIds || []), [calculation.overlapIds]);
  const persistedPlan = useMemo(
    () => buildSchedulePlanForSave(planner, { className, subject, schedule, startDate, endDate }),
  ...
  const recalcPeriods = (periods, selectedDays = planner.selectedDays, globalSessionCount = planner.globalSessionCount) =>
    (periods || []).map((period, index) => ({
      ...
      endDate: period.startDate
        ? computeAutoEndDate(period.startDate, selectedDays, globalSessionCount) || period.endDate || ''
        : period.endDate || '',
    }));

  const handleSubjectSelect = (nextSubject) => {
    const nextPlan = commitPlan({ ...planner, subject: nextSubject }, { subject: nextSubject });
    onSubjectChange?.(nextSubject, nextPlan);
  };

  const handleClassNameInput = (nextClassName) => {
    const nextPlan = commitPlan({ ...planner, className: nextClassName }, { className: nextClassName });
  ...
  const handleDayToggle = (dayValue) => {
    ...
    commitPlan({
      ...
      billingPeriods: recalcPeriods(planner.billingPeriods, nextDays, planner.globalSessionCount),
    });
  };
  ...
  const handleSessionCountChange = (nextCount) => {
    commitPlan({
      ...
      billingPeriods: recalcPeriods(planner.billingPeriods, planner.selectedDays, nextCount),
    });
  };

  const handlePeriodChange = (periodId, patch, { recalcEndDate = false } = {}) => {
    const nextPeriods = planner.billingPeriods.map((period) => {
      ...
      if (recalcEndDate && nextPeriod.startDate) {
        nextPeriod.endDate = computeAutoEndDate(nextPeriod.startDate, planner.selectedDays, planner.globalSessionCount);
      }
  ...
  const handleAddPeriod = () => {
    const lastPeriod = planner.billingPeriods[planner.billingPeriods.length - 1];
    const suggestedStartDate = getSuggestedNextStartDate(lastPeriod?.endDate, planner.selectedDays);
    const suggestedEndDate = suggestedStartDate
      ? computeAutoEndDate(suggestedStartDate, planner.selectedDays, planner.globalSessionCount)
      : '';
  ...
  const handleRemovePeriod = (periodId) => {
    if (planner.billingPeriods.length <= 1) {
      return;
    }
    ...
    commitPlan({
      ...planner,
      billingPeriods: planner.billingPeriods.filter((period) => period.id !== periodId),
    });
  };
  ...
  const handleSessionMemoChange = (date, memo, isForced) => {
    ...
    const draft = updateSessionStateDraft(current, {
      nextState: current?.state === 'force_active' ? 'active' : current?.state || 'active',
      memo,
  ...
  const handleCalendarToggle = (date, meta) => {
    ...
    if (currentState === 'force_active') {
      const nextStates = { ...(planner.sessionStates || {}) };
      delete nextStates[date];
      commitPlan({ ...planner, sessionStates: nextStates });
      return;
    }

    if (!currentState) {
      if (meta?.hasSession) {
        handleSessionStateChange(date, 'exception', false);
      } else {
        handleSessionStateChange(date, 'active', true);
      }
      return;
    }

    if (currentState === 'exception') {
      handleSessionStateChange(date, 'tbd', false);
      return;
    }

    if (currentState === 'tbd') {
      handleSessionStateChange(date, 'active', false);
      return;
    }

    handleSessionStateChange(date, 'exception', currentState === 'force_active');
  };
        ...
        {entries.map((entry) => (
          ...
          >
            ...
            >
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>
                  {entry.sessionNumber ? `${entry.sessionNumber}회차` : '회차 외 일정'}
                </div>
                <div style={{ fontSize: 14, fontWeight: 800 }}>{formatPlannerDateLabel(entry.date)}</div>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                <ToggleChip
                  ...
                  onClick={() => handleSessionStateChange(entry.date, 'active', entry.isForced)}
                >
                  진행
                </ToggleChip>
                <ToggleChip
                  ...
                  onClick={() => handleSessionStateChange(entry.date, 'exception', entry.isForced)}
                  tone="danger"
                >
                  휴강
                </ToggleChip>
                <ToggleChip
                  ...
                  onClick={() => handleSessionStateChange(entry.date, 'tbd', entry.isForced)}
                  tone="warning"
                >
                  미정
                </ToggleChip>
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: entry.state === 'exception' ? 'minmax(0, 1fr) minmax(0, 140px)' : 'minmax(0, 1fr)',
                gap: 8,
                marginTop: 10,
              }}
            >
              <input
                ...
                placeholder="메모"
                value={entry.memo || ''}
                onChange={(event) => handleSessionMemoChange(entry.date, event.target.value, entry.isForced)}
                style={{ minWidth: 0 }}
              />
              {entry.state === 'exception' ? (
                <input
                  ...
                  onChange={(event) => handleMakeupDateChange(entry.date, event.target.value, entry.isForced)}
                  style={{ minWidth: 0 }}
                />
              ) : null}
            </div>
      ...
      <div className="planner-top-grid">
        <section className="planner-panel" style={{ display: !isCompact || mobileSection === 'setup' ? 'block' : 'none' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {SUBJECT_OPTIONS.map((option) => (
                  <ToggleChip key={option} active={subject === option} onClick={() => handleSubjectSelect(option)}>
                    {option}
                  </ToggleChip>
                ))}
              </div>
            ...
            <div>
              <SectionLabel tooltip="공개 일정표와 이미지 저장 제목에 사용됩니다.">공식 수업명</SectionLabel>
              <input
                type="text"
                className="styled-input"
                value={className || ''}
                onChange={(event) => handleClassNameInput(event.target.value)}
                placeholder="예: 대기고3"
              />
            </div>

            <div>
              <SectionLabel tooltip="선택한 요일 수를 기준으로 기본 회차가 자동 추천됩니다.">수업 요일</SectionLabel>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {DAY_OPTIONS.map((day) => (
                  <ToggleChip
                    key={day.value}
        ...
        <section className="planner-panel" style={{ display: !isCompact || mobileSection === 'periods' ? 'block' : 'none' }}>
          ...
          >
            ...
            <button type="button" className="btn-secondary" onClick={handleAddPeriod}>
              <Plus size={16} />
              기간 추가
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {planner.billingPeriods.map((period) => {
              const entries = calculation.editorEntriesByPeriod?.[period.id] || [];
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <div style={{ fontSize: 15, fontWeight: 800 }}>{period.label}</div>
                        <span
                          style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <button
                        type="button"
                        className="btn-icon"
                        onClick={() => setExpandedPeriods((current) => ({ ...current, [period.id]: !expanded }))}
                  ...
                  <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
                    <select
                      className="styled-input"
                      value={period.month}
                      onChange={(event) => handlePeriodChange(period.id, { month: Number(event.target.value) || 1 })}
                    >
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
                      <input
                        ...
                        onChange={(event) => handlePeriodChange(period.id, { startDate: event.target.value }, { recalcEndDate: true })}
                        style={{ minWidth: 0 }}
                      />
                      <input
                        type="date"
                  ...
                  {overlapIdSet.has(period.id) ? (
                    ...
                    >
                      다른 기간과 날짜가 겹칩니다. 시작일 또는 종료일을 조정해 주세요.
                    </div>
                  ) : null}

...
export default function TeacherWeeklyView({
  ...
  const renderGrid = (teacherEntry) => {
    ...
    return (
      <section className={`card ${isAllView ? 'view-all-container' : ''}`} key={teacherEntry.key} style={{ position: 'relative', padding: 24, marginBottom: isAllView ? 0 : 24, breakInside: 'avoid' }}>
        <button
          type="button"

...
export default function DailyTeacherView({
  ...
  const layoutMetrics = useMemo(
    () =>
      getTimetableLayoutMetrics({
        ...
        columnCount: teacherEntries.length,
      }),
    [teacherEntries.length, visibleTargetsToRender.length, visibleWindow.visibleSlotCount]
  );
  ...
  const handleSaveFullImage = useCallback(async () => {
    if (!scheduleRef.current || visibleTargetsToRender.length === 0) {
      toast.info('저장할 일별 선생님 시간표가 없습니다.');
      return;
    }

    try {
      await exportElementAsImage(scheduleRef.current, 'daily-teacher-overview.png', {
        preset: visibleTargetsToRender.length > 1 ? 'a4-landscape' : 'a4-portrait',
      });
      toast.success('현재 일별 선생님 시간표를 이미지로 저장했습니다.');
    } catch {
  ...
  useEffect(() => {
    if (!exportRequest || exportRequest.view !== 'daily-teacher') {
      return;
    }
  ...
  const activeDay = effectiveDesktopDays[0] || selectedDay;
    setCreateState({
      summary: { day: activeDay, start: range.start, end: range.end, fixedAxisLabel: '\uC120\uC0DD\uB2D8', fixedAxisValue: teacher },
      ...
      slot: { day: activeDay, ...range, teacher },
    });
  };
  ...
  }, [classes, effectiveDesktopDays, selectedDay, teacherEntries, timeSlots]);
  ...
  return (
    <div className="animate-in">
      ...
      {teacherEntries.length === 0 ? (
        ...
        <div className="card" style={{ padding: 28 }}>
          <EmptyState message="현재 필터에 맞는 일별 선생님 시간표가 없습니다." />
        </div>
      ) : isMobile ? (

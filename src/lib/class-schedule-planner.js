function parseSchedule(scheduleStr = "") {
  const text = String(scheduleStr || "").trim();
  if (!text) {
    return [];
  }
...
function clampMonth(value) {
  ...
  const monthLabelMatch = rawValue.match(/^(\d{1,2})\s*월?$/);
  const number = yearMonthMatch
    ? Number(yearMonthMatch[1])
    : monthLabelMatch
      ? Number(monthLabelMatch[1])
      : Number(value);
...
function getRecommendedSessionCount(selectedDays = []) {
  const weeklyCount = uniqueSortedDays(selectedDays).length;
  return weeklyCount > 0 ? weeklyCount * 4 : 0;
}
...
function normalizeSessionStateEntry(entry) {
  ...
  return {
    ...
    makeupMemo: String(entry.makeupMemo || "").trim(),
...
function buildSessionStateDraft(
  existing,
  { nextState, memo, makeupMemo, makeupDate, isForced },
) {
  ...
  const nextMakeupMemo = makeupMemo ?? current.makeupMemo ?? "";
  ...
  if (nextState === "active") {
    ...
    if (!nextMemo && !nextMakeupMemo && !nextMakeupDate) {
      return null;
    }
...
export function applyCalendarDateToggle(planInput, dateString, meta = {}) {
  const currentEntry = planInput?.sessionStates?.[dateString] || {};
...
export function applyCalendarDateSubstitution(planInput, sourceDate, targetDate) {
  ...
  return applySessionStateChange(planInput, sourceDate, {
    ...
    makeupMemo: planInput?.sessionStates?.[sourceDate]?.makeupMemo || "",
...
export function getNextBillingPeriodMonth(period = {}) {
  const currentMonth = clampMonth(
    period.month || period.label || extractMonth(period.startDate || period.endDate),
  );
  return currentMonth >= 12 ? 1 : currentMonth + 1;
}

function buildDeterministicBillingPeriodId(period = {}, index = 0, resolvedEndDate = "") {
  const explicitId = String(period.id || "").trim();
  if (explicitId) {
    return explicitId;
  }

  const startDate = String(period.startDate || "").trim() || "undated";
  const endDate = String(resolvedEndDate || period.endDate || "").trim() || "open";
  const month = clampMonth(period.month || extractMonth(startDate));
  return `period-${String(index + 1).padStart(2, "0")}-${startDate}-${endDate}-${month}`;
}
...
function canReuseSessionForBillingPeriod(existingSession, billingId) {
  const existingBillingId = String(existingSession?.billingId || "").trim();
  const nextBillingId = String(billingId || "").trim();

  return !existingBillingId || !nextBillingId || existingBillingId === nextBillingId;
}
...
function buildExistingSessionMaps(rawSessions = [], textbooks = []) {
  ...
  (rawSessions || []).forEach((session) => {
    ...
    if (
      ...
      const billingAndNumberKey = `${normalized.billingId}:${normalized.sessionNumber}`;
      const queue = countedByBillingAndNumber.get(billingAndNumberKey) || [];
      queue.push(normalized);
      countedByBillingAndNumber.set(billingAndNumberKey, queue);
    }

    if (normalized.date) {
      const queue = rawByDate.get(normalized.date) || [];
      queue.push(normalized);
      rawByDate.set(normalized.date, queue);
    }
  });
...
function takeSessionFromQueue(queueMap, key, options = {}) {
  const queue = queueMap.get(key);
  if (!queue || queue.length === 0) {
    return null;
  }

  const { claimedSessionIds = null, predicate = null } = options || {};
  ...
  while (queue.length > 0) {
    ...
    if (claimedSessionIds && match.id && claimedSessionIds.has(match.id)) {
      continue;
    }
    if (predicate && !predicate(match)) {
      continue;
    }
    if (claimedSessionIds && match.id) {
      claimedSessionIds.add(match.id);
    }
    if (queue.length === 0) {
      queueMap.delete(key);
    }
  ...
  periodMeta.forEach((period) => {
    ...
    baseEntries.forEach((entry) => {
      ...
      if (countsTowardTotal && sourceDate) {
        existing = takeSessionFromQueue(existingMaps.countedBySourceDate, sourceDate, {
          claimedSessionIds: claimedExistingSessionIds,
          predicate: (session) => canReuseSessionForBillingPeriod(session, entry.billingId),
        });
      }

      if (!existing) {
        existing = takeSessionFromQueue(existingMaps.rawByDate, entry.date, {
          claimedSessionIds: claimedExistingSessionIds,
          predicate: (session) => canReuseSessionForBillingPeriod(session, entry.billingId),
        });
      }
